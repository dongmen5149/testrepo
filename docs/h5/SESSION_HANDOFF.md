...
## ⚡ "영웅서기5 다음 내용 진행해줘" → **Round 90 (SMAF audio / Spirit placeholder / 실 디바이스 빌드)**

차기 세션 즉시 할 일:
1. 환경 확인: `PYTHONIOENCODING=utf-8 python tools/verify_godot_project.py` → `0 warnings` 확인
2. 회귀 테스트 (선택): `python tools/h5_test_launcher_icons.py` → `R89 ... ALL PASSED`
3. **R90 추천 작업** (자율 가능 우선순위):
   - 옵션 A: **Godot Editor 실 빌드 검증** (사용자 작업, R86 export preset + R89 launcher icons 로 APK 빌드 + 실 디바이스)
   - 옵션 B: SMAF audio playback (A 95→99%, +0.3%p) — SMAF .mmf 파일 디코더 + AudioStream Godot 통합
   - 옵션 C: Spirit desc placeholder resolution (`}#NN%|` → stats 값 치환, spirit layout 별도 매핑, F +0.5%p)
   - 옵션 D: ProcTargetEffectSkill cascade 정밀 (R81 후속, RE +0.3%p)
   - 옵션 E: Save round-trip 실 통합 검증 (F 0→30, 출시 +0.5%p)

**전체 진척 (R89 시점, 8 카테고리 가중평균)**:
