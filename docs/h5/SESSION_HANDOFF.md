...
업데이트: 2026-05-09 (Round 10 — 한글 stat label 위치 매핑 + DrawPropertyMenu 동적 dispatch 확인 + calc_sk[3]/[4] = V[41]/V[156] 추출)
...
### 1. V[112..116] 5 stat 정확 라벨 식별 (Round 10 부분 진척, 추가 단서 필요)

Round 10 에서 다음 단서 누적:
- 한글 stat 라벨이 .so 가 아닌 VFS `00017_488ab1c6.json` 에 분산 (20-stat sequence 발견).
- StateInGameMenu::DrawPropertyMenu 가 register-indirect dynamic — 정적 매핑 어려움.
- CalcStatusComputation 의 calc_sk 호출은 V[41]/V[156] (item bonus 영역) → V[112..116] 와 무관.

다음 단계 옵션:
- (a) Save 데이터 구조 분석 — save 파일 dump 후 0x278..0x282 (V[111..116]) 영역의
  실제 game-saved 값을 한글 UI 표시값과 매칭 (가장 직접).
- (b) DrawPropertyMenu 의 GOT/literal pool indirect lookup 패턴 추적 —
  cache offset table 과 string idx table 동시 풀어서 N:N 매칭.
- (c) c_csv_buildup.json (buff/buildup csv) 분석 — buff effect type N → 한글 stat
  설명 매핑 (한글 corpus 의 "크리티컬 +5" 등 string format 추적).
- (d) 한국어 stat 라벨 후보 (명중률/회피율/크리티컬/정확도/마법적중) 를 임시 라벨로
  사용하고, 실제 게임 동작 비교로 검증.
