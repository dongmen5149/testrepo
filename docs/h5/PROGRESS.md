...
| **R89: Android launcher icons asset (192×192 + 432×432 adaptive)** | R86 의 `launcher_icons` 3 슬롯 (`main_192x192` + `adaptive_foreground_432x432` + `adaptive_background_432x432`) 빈 칸을 PIL 로 자동 생성. **`tools/h5_make_launcher_icons.py` 신규 (147 줄)**: PIL `Image.new('RGBA')` + `ImageDraw` 로 icon.svg 디자인 (검은 BG + 황금 별 + 은 검 + "HERO 5") 재현. **3 helper 함수**: `make_main_192()` (legacy launcher, BG #1a1a2e + 원 + 검 + 별 + "HERO 5" 텍스트), `make_adaptive_foreground_432()` (Android 8.0+ adaptive 전경, BG 투명 + 중앙 264×264 안전 영역에 검 + 별 + "5"), `make_adaptive_background_432()` (전체 검정+자주 + 황금 테두리 원). `_draw_sword` (수직 검: blade + tip triangle + crossguard + grip) + `_star_points` (5-pointed star 좌표 계산) 공통 helper. `_load_font` 가 Windows arialbd.ttf / arial.ttf / segoeui.ttf / DejaVu Bold (Linux) / Arial Bold (Mac) 순서로 fallback. **3 PNG 생성** (assets/launcher_icons/): main_192×192 (2844B) / adaptive_foreground_432×432 transparent BG (2297B) / adaptive_background_432×432 (3238B). **export_presets.cfg 정정**: Debug + Release 양쪽 preset 의 3 launcher_icons 슬롯이 `res://assets/launcher_icons/*.png` 참조하도록 변경 (이전 `""` 빈 칸 → 정확 경로). `tools/h5_test_launcher_icons.py` 신규 — h5_make_launcher_icons.py PIL + 3 helper + sword/star 함수 + 3 PNG 파일 존재 + 정확 크기 (192×192 / 432×432 / 432×432) + IHDR direct read + alpha 채널 (main opaque / foreground transparent / background opaque) + 안전영역 (132..300) 에 non-transparent pixel 존재 + export_presets.cfg 2 preset × 3 슬롯 채워짐 + res:// 경로의 디스크 파일 매칭 + R86 회귀 (2 preset / gradle_build / arm64 / package / immersive / 11 permissions / Release compress) + icon.svg + project.godot icon 참조 + .gitignore *.keystore 잔존, **10/10 통과**. R82/R83/R84/R85/R86/R87/R88 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: G 카테고리 55% → **65%** (launcher_icons asset 0→90 신규, Distribution 80 + Build doc 90 + Packaging 60 유지). 종합 가중평균 81.17% → **81.97% (↑0.80%p)**. 출시 가능 기준 (D+E+G 가중) 76.0% → **76.8%**. Godot 92-95% 유지 (G 는 Godot 외) |
| **R88: Spirit desc EUC-KR 디코딩 — 16/16 한국어 텍스트 추출** | R87 explicit field 추출 후속. R77 file layout 의 desc_string 영역 (bytes[48..48+desc_len], EUC-KR 인코딩) 후처리 디코딩. **`tools/converter/decode_h5_skill_desc.py` 신규**: c_csv_skill_05.json 각 record 에 `desc_text` 필드 in-place 추가 (16/16 디코딩 성공, 평균 68.9 글자 / min 33 / max 88). EUC-KR codec + STATS_AREA_SIZE=48 / DESC_LEN_OFFSET=0x2f 명시 상수. **샘플**: spirit #0 암흑탄 = "거대한 암흑탄을 발사하여;정령마력 }#05%|의;피해를 준다.;마법공격력으로;위력이 증가." / spirit #2 영혼의회복 = "버프 스킬.;사용 즉시;대량의 HP를 회복하고;}#08초|동안 소량의 HP가;자동회복된다." / spirit #7 정신감응 = "패시브 스킬.;전투시 정령 게이지가;충전되는 양이;}1.5배| 증가한다." `;` = 줄바꿈, `}#NN%|` = stat placeholder (spirit layout 다르므로 R89+ 정밀화). **`game_data._ensure_spirit_skills_loaded()` 보강**: `desc_text` → `entry["desc"]` 채움 (이전엔 ""). 이로써 `skill_info(5, sid).desc` 가 한국어 desc 반환. **`game_data.resolve_skill_desc` class_5 분기**: 호출 시 `_ensure_spirit_skills_loaded()` 자동 호출 (skills.json 만 로드된 상태에서도 spirit desc 접근 가능). **`battle_system._skill_data` 정정**: 반환 dict 에 `"desc": rec.get("desc", "")` 추가 (외부 UI 조회용). **SKILL action 로그 보강**: Sorcerer (class_id=4) + spirit skill 발동 시 `log_message.emit("  ▸ " + desc.split(";")[0])` 로 첫 segment 한 줄 노출 (예: "  ▸ 거대한 암흑탄을 발사하여"). `tools/h5_test_spirit_desc_decode.py` 신규 — decode_h5_skill_desc.py 존재 + EUC-KR codec + 48B/0x2f 상수 + 16/16 desc_text 보유 + 16/16 한글 포함 + 3 sample 키워드 (암흑탄/영혼의회복/정신감응) + R77 sub-rel offset 회귀 (effect_type 분포 0=5/2=9/7=2) + game_data desc_text → entry["desc"] wiring + resolve_skill_desc class_5 fallback + battle_system desc 필드 반환 + SKILL action 로그 marker (▸ + split(";")[0]) + 평균 desc 길이 통계, **13/13 통과**. R75-R87 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: F 카테고리 40% → **50%** (Spirit desc 0→90 신규, 16 spirit 한국어 노출). 종합 가중평균 80.17% → **81.17% (↑1.00%p)**. Godot 92-95% 유지. 출시 82% 유지 (F 는 출시 가중치 외) |
...
**현 위치** (2026-05-20, R108): 상세 정의는 **[COMPLETION.md](COMPLETION.md)** 참고.

| 지표 | 수치 | 의미 |
|------|------|------|
| **종합 개발** (8카테고리) | **87.6%** | RE·데이터·Godot·자동테스트 포함 전체 진척 |
| **클라이언트 출시 축** (D+E+G) | **~88%** | APK 패키징·씬·코어 (기기 미검증 전제) |
| **베타 서비스 오픈** (원작 동등 플레이) | **~50%** | 동일 화면·시나리오 정상 플레이 = 100% |
| **기술 스모크** (APK·demo 30분) | **~61%** | 시나리오 무관 — adb 0% |
| **오픈 베타 체감** | **~58%** | SMAF·본편·다기기 QA 미완 |

데이터 RE **~100%** / .so **~99%** / Godot 구현 **93-96%**. 레거시 `출시 가능 79.60%` 는 R82–R103 증분 로그용 — 신규 보고는 COMPLETION.md 사용.

업데이트: 2026-05-20 (완성도 문서 정비 + R108 반영) — **[COMPLETION.md](COMPLETION.md)** 신규: 4지표 정의·혼동 방지·베타 갭 우선순위. `tools/h5_test_completion_doc.py` 검증.

업데이트: 2026-05-20 (Round 112 종료) — **skill desc 데이터 quirk 흡수 — 188 skill 전수 검증, 62 잔존 raw marker → 1 outlier**. R111 까지의 placeholder/render 시스템 전수 audit (`tools/h5_r112_skill_desc_audit.py`) 결과 188 skill 중 **62건 잔존 raw marker** 발견. 4 fix 적용: (1) **디코더 fix** `tools/converter/decode_h5_skill.py` — 한글 직전 `}` (0x7d) backtrack — passive desc `}<active_skill>|` form 의 leading `}` 보존, skills.json 재생성 → 58건 해결. (2) **중첩 `}` 평탄화** — 봉쇄/섬광탄 `}<text>}<num>|` 의 inner `}` 제거 → `[시야를 1]로`. (3) **`{` alt close marker** — 쐐기탄 `}민첩 12당 1{의` (bit-flip 가설, `{`=0x7b vs `|`=0x7c) 수용 → `[민첩 12당 1]의`. (4) **불릿 line-start 가드** — R111 의 `#NN<text>` → `• <text>` 가 포격 `사격당 |#07|의` 의 `#07` 잘못 불릿화 차단 (`out.is_empty() or out.ends_with("\\n")` 조건). 최종 audit: 62→4→3→2→**1/188** (포격 `|#07|` orphan, 원본 emphasis 의도 가능, R113+ 검토). **`skill_desc_placeholder.md` §9 신규** — 4 fix 동기 + audit 단계별 감소 표 + outlier. `tools/h5_r112_skill_desc_audit.py` + `tools/h5_test_skill_desc_quirks.py` 신규 (14 검증) — decoder backtrack + 각력 `}돌진|` sample + 3 display fix marker + 3 실 사례 + audit ≤ 1 + R90/R105/R108/R110/R111 회귀, **14/14 PASS**. R105-R111 회귀 모두 통과. verify_godot **0 warnings**. **진척률**: F 71→**72%** (R112 quirk 흡수 88 신규 — 99.5% 클린 렌더링), 종합 87.89→**87.99% (↑0.10%p)**. Godot 93-96% 유지.

이전 업데이트: 2026-05-20 (Round 111 종료) — **`{관련특성|` 섹션 헤더 + bare `#NN<text>` 스킬-링크 불릿 + #10/#11/#13 라벨 추가**. R110 의 자연스러운 UX 완성. class_0..3 desc 의 (1) `{관련특성|` 섹션 헤더 raw 노출 37건, (2) bare `#NN<text>` 스킬-링크 raw 노출 46건, (3) #10/#11/#13 unresolved 시 단순 `?` 노출 (#10 만 71회 사용) — 3종 동시 fix. **PLACEHOLDER_LABELS** 3 entry 추가 (`10: "값"`, `11: "강화"`, `13: "양"`) — 총 10 entry. **`resolve_skill_desc` indices 자동 수집** — while loop 가 `}...|` 괄호 내부의 모든 `#NN` 자동 스캔 → values dict entry 보장. **`resolve_skill_desc_display` 보강**: (1) `c == "{"` 분기 → `{관련특성|` → `▸ 관련 특성:`, (2) `c == "#"` + 2 digit → bare `#NN<text>` → `• <text>` 불릿. 돌진 예: `{관련특성|;#01돌격-스턴효과;#02돌격-밟고가기` → `▸ 관련 특성:\n• 돌격-스턴효과\n• 돌격-밟고가기`. **`help_panel.gd`** HELP_TEXT 갱신 — 10 entry label + R111 섹션/불릿 안내. **`docs/h5/RE/skill_desc_placeholder.md` §8 신규** — R111 fix 3종 + 돌진 R110/R111 비교 + 실 데이터 카운트. `tools/h5_test_placeholder_section_render.py` 신규 (14 검증) — 10 entry LABELS + while loop + `{` 분기 + `#` 분기 + Python 시뮬 + #10 fallback + R110/R109/R108/R105/R90 회귀 + 실 데이터 37 섹션 + 69 skill-link, **14/14 PASS**. R107 회귀 테스트 entry 수 7 → 10 갱신. R100-R110 회귀 모두 통과. verify_godot **0 warnings**. **진척률**: F 70→**71%** (R111 섹션+불릿 87 신규), 종합 87.79→**87.89% (↑0.10%p)**. Godot 93-96% 유지.

이전 업데이트: 2026-05-20 (Round 110 종료) — **Bracket-aware placeholder 치환 — class_0..3 skill-link 보존**. R109 자연스러운 정합성 라운드. R108 의 무차별 `result.replace("#%02d", val)` 가 class_0..3 desc 의 **bare `#NN` skill-link 참조** (예: `#01돌격-스턴효과`) 를 corruption (stats_u16[1]=0 → `0돌격-스턴효과`). 실 데이터 영향: class_0..3 에서 **46개 skill-link corruption** vs **244개 진짜 placeholder** (괄호 내부 `}#NN<unit>|` + 라벨 `}SP #07|`). R110 fix = `_replace_placeholders_in_brackets(desc, values)` helper 신규 — `}...|` 괄호 내부 #NN 만 치환, bare #NN 보존. **`resolve_skill_desc` 재구성**: indices 산출 → values dict 미리 계산 → helper 단일 호출. **`skill_desc_placeholder.md` §7 신규**: 동기 + R108 corruption vs R110 보존 비교 + 라벨 placeholder 동작 + 실 데이터 카운트. `tools/h5_r110_bracket_aware_audit.py` + `tools/h5_test_placeholder_bracket_scope.py` 신규 (14 검증) — helper + resolve 위임 + 무차별 replace 제거 + 돌진 skill-link 보존 + 도발 `}SP #07|` 라벨 치환 + R105/R108/R109 회귀 + 실 데이터 카운트 (69 → 보존 / 244 유지), **14/14 PASS**. R105 회귀 (h5_test_placeholder_guard) 의 `display_val` 변수 검사 → R108/R110 위임 허용으로 갱신. R100/R101/R104/R105-R109 회귀 모두 통과. verify_godot **0 warnings**. **진척률**: F 69→**70%** (R110 bracket-aware 85 신규 — 46건 corruption 차단), 종합 87.69→**87.79% (↑0.10%p)**. Godot 93-96% 유지.

이전 업데이트: 2026-05-20 (Round 109 종료) — **Placeholder label/unit 분리 + #12 잘못된 매핑 제거**. R108 의 자연스러운 정합성 라운드 — R106 label 이 unit (`%`, `초`) 을 포함 → desc 본문 unit 과 중복 노출 (`[?(공격%)%]` / `[?(지속초)초]`) 차단 + #12 의 `primary_u16` 매핑이 spirit #6 폭발 `}#12초|` 를 damage% (300) 로 잘못 해석하던 케이스 정정. **`PLACEHOLDER_LABELS`** 7 entry 단위 제거: `"효과%"→"효과"`, `"공격%"→"공격"`, `"마법%"→"마법"`, `"지속초"→"지속"`, `"쿨초"→"쿨"`, `"배수"→"수치"` (#7 `"MP"` 그대로). **`PLACEHOLDER_STAT_SOURCE`** 6 entry (12 제거) — #12 미매핑 시 stats_u16[12] fallback (대부분 garbage > 500) → `[?(수치)초]` 안전 표시. **`help_panel.gd`** R109 안내 갱신 — `?(공격)%` / `?(지속)초` 형식 예시 + 7 R109 label. **`docs/h5/RE/skill_desc_placeholder.md` §6 신규**: R106→R109 label 비교 표 + #12 매핑 정정 사유 + spirit 16 record 실 값 (primary/secondary/mp/cooldown/fid1/fid2 + placeholder 분포). **관측**: spirit primary_u16 6/16 비-0, secondary_u16 16/16 = 0 (`#08초` 모두 0 표시 — duration RE 미완), cooldown byte 0x0d=93 동일 상수. `tools/h5_r109_spirit_placeholder_audit.py` + `tools/h5_test_placeholder_label_unit.py` 신규 (14 검증) — 단위 제거 + R106 form 잔존 0 + 12 제거 + 폭발 stats_u16[12]=32569 garbage 가드 + 암흑탄 #05=400 회귀 + Python 시뮬 (`[?(공격)%]` / `[?(지속)초]` / `[?(수치)초]` + R106 단위 중복 form 미발생) + R105 THRESHOLD + R107/R108 helper 회귀 + RE §6 검증, **14/14 PASS**. R105-R108 회귀 테스트 5건 (h5_test_placeholder_{guard,labels,formula,help_panel_placeholder,help_panel_sync}) 의 R106 form 잔존 검사를 R106/R109 form 둘 다 허용으로 갱신 — 모두 PASS. verify_godot_project **0 warnings**. **진척률 갱신**: F 68→**69%** (R109 label/unit 분리 + #12 정정 82 신규), 종합 87.59→**87.69% (↑0.10%p)**. 출시 가능 기준 79.60% 유지 (F 는 출시 가중치 외). Godot 93-96% 유지.

이전 업데이트: 2026-05-20 (Round 108 종료) — **Placeholder Formula + explicit field 통합 (1차)**. R107 HelpPanel 후속, R105-R106 `stats_u16[NN]` 가 garbage 인 문제에 실 해결 경로 추가. `game_data.gd`: `PLACEHOLDER_STAT_SOURCE` (7 NN) + `eval_placeholder_stat` + `_calc_placeholder_formula` + `_placeholder_player_ctx` — 해석 순서 FormulaVM.calc(formula_id_1/2) → spirit `primary_u16`/`mp_cost`/`cooldown` (R87) → reasonable stats → `?(label)`. **spirit #0 암흑탄 `}#05%|` → `400%`** (primary_u16=400, 이전 7728 또는 `?(공격%)`). `PLACEHOLDER_LABELS` 에 #06 `마법%` 추가. `help_panel.gd` R108 안내 갱신. `docs/h5/RE/skill_desc_placeholder.md` §5 R108. `tools/h5_test_placeholder_formula.py` 신규. R105-R107 회귀 통과. verify_godot_project **0 warnings**. **진척률**: F 65→**68%**, 종합 87.49→**87.59% (↑0.10%p)**. Godot 93-96% 유지.

이전 업데이트: 2026-05-20 (Round 107 종료) — **HelpPanel 에 placeholder label 섹션 신규 — 사용자 UX 인지 보강**. R105 (UNREASONABLE 가드) + R106 (label fallback) 의 placeholder UX 가 사용자 입장에서 "`[?(공격%)]` 라는 표기가 왜 뜨는가" 가 도움말 어디에도 없어 인지 불가. R107 = `help_panel.gd::HELP_TEXT` 끝에 "[b]스킬 설명 placeholder[/b]" 섹션 추가: 6 PLACEHOLDER_LABELS entry (#04→효과% / #05→공격% / #07→MP / #08→지속초 / #09→쿨초 / #12→배수) + THRESHOLD 500 정상 범위 + 미매핑 `?` fallback 명시. `game_data.gd::PLACEHOLDER_LABELS` 와 1:1 일치 (drift 방지 검증). `tools/h5_test_help_panel_placeholder.py` 신규 (15 검증): 섹션 헤더 + 6 NN/label entry + THRESHOLD 멘션 + 미매핑 fallback + game_data dict 1:1 매칭 + R106/R105/R94/R93 회귀 + docstring R107 marker + bbcode/toggle 회귀 + 섹션 순서 (Title 뒤), **15/15 PASS**. R94 (help_panel_sync) + R106 (placeholder_labels) 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: H 74→**75%** (HelpPanel 사용자 가시화 95 신규 — 6 PLACEHOLDER_LABELS 인지 가능), 종합 87.39→**87.49% (↑0.10%p)**. 출시 가능 기준 79.60% 유지 (H 는 출시 가중치 외). Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 106 종료) — **Placeholder NN → 의미 label 매핑 (R75 convention)**. R105 의 `?` 단순 fallback 보강. `PLACEHOLDER_LABELS` 6 entry (4효과%/5공격%/7MP/8지속초/9쿨초/12배수). garbage 값 시 `?(공격%)` 처럼 의미 label 노출. RE/skill_desc_placeholder.md §4 갱신 (R106 label 매핑 표 + 예시). 미매핑 NN 은 `?` 유지. `tools/h5_test_placeholder_labels.py` 신규 (15 검증): 6 entry + label fallback 분기 + Python 시뮬 6 case + 합성 시뮬 + R90-R105 회귀, **15/15 PASS**. R105 회귀 테스트는 ternary → label form 허용. R90-R105 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: F 63→**65%** (label fallback 75 신규), 종합 87.19→**87.39% (↑0.20%p)**, 출시 79.60% 유지. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 105 종료) — **Spirit placeholder 실 stat source RE 분석 + UNREASONABLE 가드**. R90 의 `}#NN<unit>|` 가 garbage 값 (7728%/41008%/-1초) 표시되던 원인 분석. spirit 16 + class_0 43 의 stats_u16 인덱스 5/6/8/12 가 모두 동일 상수 (7728/23808/0/32569) — file 헤더이지 placeholder source 아님. R75 가설 (stats_u16[NN] 매핑) 잘못됨, 실 source = Formula::calc 런타임 (R106+ 통합 작업). resolve_skill_desc 에 `PLACEHOLDER_UNREASONABLE_THRESHOLD=500` 가드 추가 — 값 > 500 시 `?` 로 fallback (garbage 노출 차단). `docs/h5/RE/skill_desc_placeholder.md` 신규 (5 섹션: 요약/실측/source 추정/가드/후속). `tools/h5_test_placeholder_guard.py` 신규 (15 검증, Python 시뮬 + spirit JSON 영향 확인), **15/15 PASS**. R90/R91-R104 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: F 60→**63%** (placeholder source RE + 가드 70 신규), 종합 86.89→**87.19% (↑0.30%p)**, 출시 79.60% 유지. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 104 종료) — **SettingsPanel `_on_mute_toggled` 중복 `_save_config` 제거 — 단일 source-of-truth**. R103 핸들러 중앙화 마무리. 사용자가 체크박스 클릭 시 (1) `_on_mute_toggled` inline save + (2) signal chain save 의 2회 저장 발생을 1회로 정리. `_on_mute_toggled` 2줄→1줄 (`Audio.set_muted(on)` 만), save 는 mute_changed → demo handler chain 단일 경로. `tools/h5_test_dedup_save.py` 신규 (14 검증, Python 시뮬 포함): inline 제거 + chain save 잔존 + 다른 슬라이더/체크 save 유지 + 단일 save 시뮬 + cycle 가드 + R91-R103 회귀, **14/14 PASS**. R99 회귀: inline 검사 → inline/chain 허용. R98-R103 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: H 73→**74%** (코드 품질 향상), 종합 86.84→**86.89% (↑0.05%p)**, 출시 79.60% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 103 종료) — **HUD AudioIndicator 클릭 토글 + mute_changed 단일 핸들러 중앙화**. R102 후속. hud.tscn AudioIndicator mouse_filter=0 + tooltip 갱신. hud.gd: gui_input → 좌클릭 release → Audio.toggle_mute(). demo.gd: Audio.mute_changed.connect → `_on_audio_mute_changed(muted)` 핸들러가 sync_mute_check + _save_config + Toast warn/info 통합. F8 핸들러 ~7줄 → `Audio.toggle_mute()` 1줄로 단순화. 3 entry point (F8 / HUD click / SettingsPanel checkbox) 모두 동일 signal chain. `tools/h5_test_hud_indicator_click.py` 신규 (13 검증): tscn mouse_filter + tooltip + hud gui_input handler + demo mute_changed 연결 + signal handler 통합 + F8 단순화 + R98-R102 회귀, **13/13 PASS**. R98/R99/R102 회귀 테스트는 F8 inline 검사 → inline 또는 signal handler 허용으로 갱신. R98-R102 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: E 94→**95%** (Demo 88→90 + Audio 70→80), 종합 86.72→**86.84% (↑0.12%p)**, 출시 가능 기준 79.48→**79.60%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 102 종료) — **HUD AudioIndicator + Audio.mute_changed signal**. R101 후속, audio UX 마무리. audio_manager.gd: `signal mute_changed(muted: bool)` + set_muted 의 changed 가드 + emit. hud.tscn AudioIndicator Label 신규 (offset_right=316 우상단, F8 tooltip). hud.gd: @onready var + mute_changed.connect + _on_mute_changed handler + ON("♪" 연두) / OFF("🔇" 빨강) 4 상수 + _ready 초기 상태 적용. polling 없이 signal 동기화. `tools/h5_test_hud_audio_indicator.py` 신규 (15 검증): signal + changed 가드 + 노드 + handler + 상수 + Python 시뮬 (중복 set emit 없음) + R96-R101 회귀, **15/15 PASS**. R98-R101 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: E 93→**94%** (Audio 60→70), 종합 86.60→**86.72% (↑0.12%p)**, 출시 가능 기준 79.36→**79.48%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 101 종료) — **Audio bus layout 분리 (Master → Master+BGM+SFX 3 bus)**. R100 후 첫 라운드. default_bus_layout.tres 신규 + project.godot [audio] 섹션. audio_manager.gd: BGM_BUS_NAME/SFX_BUS_NAME 상수 + bus index lookup + Master fallback + AudioServer.set_bus_volume_db/set_bus_mute 사용 + player.volume_db = 0 (bus 가 dB 관리) + _fade_swap fade-in target bus-aware. 외부 API 시그니처 무변경 → R97-R99 호출자 무수정. `tools/h5_test_audio_bus_layout.py` 신규 (16 검증), **16/16 PASS**. R97-R99 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: D 92→**93%** (AudioManager bus-level 정밀 98), E 92→**93%** (Audio 50→60), 종합 86.28→**86.60% (↑0.32%p)**, 출시 가능 기준 79.04→**79.36%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 100 종료) — **밀레스톤 결산 — R82-R99 18 라운드 + 잔여**. `docs/h5/MILESTONE_R100.md` 신규 (5 섹션: 누적 변화 표 / 카테고리 마일스톤 / 잔여 4 옵션 / 18 라운드 라인업 / 핵심 RE 종결 사실). R82 73.01% → R99 86.07% (+13.06%p) 정리, 가장 큰 도약 = E +37%p (Scene/UX) / F +54%p (누락 시스템). 이론 천장 ≈ 88.2% (R100+ 4 옵션 모두 완료 시). `tools/h5_test_milestone_r100.py` 신규 (15 검증): 5 섹션 + 종합 수치 + 18 라운드 + 8 카테고리 + 잔여 옵션 + 이론 천장 + 6 핵심 사실 + R91-R99 회귀, **15/15 PASS**. R99 회귀 통과. verify_godot_project **0 warnings**. **진척률 갱신**: H 70→**73%** (밀레스톤 문서 95 신규), 종합 86.07→**86.28% (↑0.21%p)**, 출시 79.04% 유지. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 99 종료) — **SettingsPanel Mute 체크박스 UI + F8 양방향 동기화**. R98 후속. settings_panel.tscn MuteCheck CheckBox 신규 (offset_top=92, "음소거 (F8)") + 기존 4 노드 재배치 (FPSCheck 96→120, Fullscreen 124→148, FPSLabel 156→180). settings_panel.gd: `@onready var mute_check` + `mute_check.toggled → _on_mute_toggled` 연결 + `_load_config` 의 `set_pressed_no_signal` 초기화 + `_on_mute_toggled = set_muted + _save_config` + **`sync_mute_check(state)` public helper** (F8 외부 silent 동기화). demo.gd F8 핸들러: `_settings.sync_mute_check(muted)` 호출 추가. 4 layer 일관 (AudioManager + Toast + 체크박스 + ConfigFile). `tools/h5_test_mute_checkbox.py` 신규 (15 검증): tscn 노드 + 재배치 + @onready + 4 함수 + sync helper + demo F8 + R91-R98 회귀, **15/15 PASS**. R98 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: E 91→**92%** (Audio 40→50), 종합 85.95→**86.07% (↑0.12%p)**, 출시 가능 기준 78.92→**79.04%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 98 종료) — **음소거 토글 F8 + ConfigFile 영속성**. R97 후속, audio UX. AudioManager 에 `_muted` + `is_muted/set_muted/toggle_mute` 3 API + set_X_volume 의 mute 가드 + play_sfx 의 mute 분기. SettingsPanel 의 ConfigFile "audio/muted" read/write + `_load_config` 시 `Audio.set_muted` 즉시 반영. demo.gd F8 키 → toggle_mute + _save_config + Toast warn/info. HelpPanel F8 추가. `tools/h5_test_mute_toggle.py` 신규 (15 검증): 4 API + 전환 + 가드 + ConfigFile + F8 + HELP + Python 시뮬 + R91-R97 회귀, **15/15 PASS**. R96 회귀: Toast 분포 == 12 → ≥12 (확장 허용). R97 회귀: play_sfx 정확 매칭 → 부분 매칭. R91-R97 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: E 90→**91%** (Audio 30→40, Mute 토글), H 65→**70%** (HelpPanel F8 추가), 종합 85.48→**85.95% (↑0.47%p)**, 출시 가능 기준 78.80→**78.92%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 97 종료) — **SettingsPanel/AudioManager 볼륨 정밀 (linear_to_db + SFX target + mute)**. R96 후속, audio 카테고리 첫 진척. 3 문제 fix: (1) 선형 `-40+(v/100)*40` dB → `linear_to_db(v/100)` 자연 곡선 (v=100→0/v=50→-6/v=10→-20dB), (2) `_sfx_target_db` 영속 변수 신규 + play_sfx 매 재생 전 재적용, (3) `_load_config` 후 즉시 AudioManager 반영. `slider_to_db` static helper + `set_bgm/sfx_volume` public API + MUTE_THRESHOLD=1.0 / MUTE_DB=-80. SettingsPanel 의 `_on_X_volume` 9줄→2줄. `tools/h5_test_audio_volume.py` 신규 (14 검증): static helper + linear_to_db + MUTE + target_db + API + 위임 + 이전 매핑 제거 + Python 시뮬 + R91-R96 회귀, **14/14 PASS**. R91-R96 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: D 91→**92%** (AudioManager 정밀 95 신규), E 89→**90%** (Audio 20→30), 종합 85.12→**85.48% (↑0.36%p)**, 출시 가능 기준 78.44→**78.80%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 96 종료) — **demo.gd 의 Toast.severity helper 마이그레이션 (12 호출) — 85% 돌파**. R95 후속, R86-R94 누적된 `preload(...).show_msg(...)` 12 호출을 R95 의 `Toast.info / success / warn` 로 마이그레이션. 분류: info×8 (퀘스트 시작/monster 디버그/타일·충돌/화면 흔들림/시스템·BGM), success×2 (퀘스트 완료 3.0s + 아이템 획득 2.0s), warn×2 (공격할 적 없음/공격 범위). preload boilerplate 제거 + 의미별 색상 (노랑/연두/주황). `tools/h5_test_toast_migration.py` 신규 (12 검증): preload 잔존 0 + Toast.X 분포 (info=8/success=2/warn=2/error=0) + 12 분류 정확성 marker + 3.0s/2.0s duration 보존 + R91-R95 회귀, **12/12 PASS**. R95 회귀 테스트는 demo 의존 → toast.gd 함수 자체 존재 검사로 변경. R91-R95 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: E 87→**89%** (Toast severity 분류 90 신규, Demo 85→88), 종합 84.88→**85.12% (↑0.24%p) — 85% 돌파**, 출시 가능 기준 78.20→**78.44%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 95 종료) — **Toast UX 정비 — severity / fade-in / stack**. R94 후속 UI/UX. toast.gd 가 같은 좌표에 겹치고 fade-in 없고 색상 일관성 부재 — Severity enum (INFO/SUCCESS/WARN/ERROR) + 4 단축 helper (info/success/warn/error) + show_severity dispatcher + `_active_toasts` static array + stack Y (110 + 32×count) + fade-in 0.18s + fade-out 0.6s tween chain + duration default 심각도별 (2.5/2.5/2.8/3.2). show_msg 시그니처 호환 유지 (R86-R94 12 호출자 그대로). `tools/h5_test_toast_ux.py` 신규 (12 검증): enum/COLORS/helper/시그니처 호환/fade tween/stack/R91-R94 회귀, **12/12 PASS**. R91-R94 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: E 85→**87%** (Toast UX 90 신규), 종합 84.64→**84.88% (↑0.24%p)**, 출시 가능 기준 77.96→**78.20%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 94 종료) — **HelpPanel 키 명세 동기화** (R82-R93 누락 키 + R93 폐기 안내 정정). HELP_TEXT 에 F6 (SaveList R92) / F10 (Quit-to-Title R82) / G (monster spawn) / SPACE (attack) / R/K/O/J/L/, (R52-58 6 패널) 신규 + R93 폐기 안내 ("slot 0 자동 로드", "슬롯 우클릭", "Shift+클릭 → 삭제") 제거 + Title 섹션 SaveListPanel 위임 안내로 갱신. `tools/h5_test_help_panel_sync.py` 신규 (11 검증): demo.gd 의 26 KEY_ 바인딩 vs 필수 set 매칭 + 10 R94 marker + 3 폐기 안내 제거 + Title Delete 안내 + R90-R93 회귀, **11/11 PASS**. R90-R93 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: H 60→**65%** (HelpPanel 키 동기화 95 신규), 종합 84.29→**84.64% (↑0.35%p)**. 출시 77.96% 유지 (H 는 출시 가중치 외).

이전 라운드 업데이트: 2026-05-19 (Round 93 종료) — **Title Continue → R92 SaveListPanel 통합 (인라인 slot UI 제거)**. R92 후속, 코드 중복 제거. title.gd 의 인라인 Slot_* 버튼 동적 생성 (_refresh_slots 35 줄) + AcceptDialog 우클릭 삭제 popup (_confirm_delete 12 줄) + _selected_slot/_on_slot_selected 등 ~50 줄 제거. 신규: `var _save_list`, _ready 에서 SaveListPanel preload+instantiate + slot_loaded → `_on_slot_loaded` 연결. `_on_continue` = `_save_list.toggle()`. `_on_slot_loaded(slot)` = SaveListPanel 이 이미 quick_load 수행했으므로 `SceneRouter.to_demo(self)` 만. `_refresh_status` = slots Label fallback 메시지 + Continue 비활성 처리만. title.tscn 무변경 (인라인 노드는 동적이었음). `tools/h5_test_title_save_list.py` 신규 (11 검증): 8 marker 제거 + _save_list 통합 + _on_continue 위임 + _on_slot_loaded 의 to_demo + quick_load 중복 없음 + _refresh_status fallback + R90/R91/R92 회귀, **11/11 PASS**. R90/R91/R92 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: E 카테고리 82→**85%** (Title 85→95), 종합 83.93→**84.29% (↑0.36%p)**, 출시 가능 기준 77.6→**77.96%**. Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 92 종료) — **SaveListPanel UI — 8 slot 선택 + 메타 표시 + 액션 버튼**. R91 round-trip 후속 UI. 그동안 1-8/Shift 키 조합만 있어 슬롯 상태 안 보였음. `scripts/ui/save_list_panel.gd` 신규 (~120 줄, class_name SaveListPanel): 8 slot 행 각각 = (Lv/Class/Gold/timestamp/playtime Label + Load/Save/Delete 버튼). AUTO slot (7) 은 save 비활성. `_format_slot` 가 H5SaveManager.load_slot() 의 timestamp/player.class_id/level/gold + play_time_sec 표시. slot_loaded signal 로 호출자에게 알림. `scenes/save_list_panel.tscn` 신규 (layer=13, 6 노드). demo.gd 통합: `var _save_list` + preload + slot_loaded → `_scene_idx=GameState.current_scene_id; _apply_scene()` 연결 + **F6 키 바인딩** (scene/map/pos sync 후 toggle). `tools/h5_test_save_list_panel.py` 신규 (11 검증): class_name/7 func/signal + H5SaveManager API + AUTO 비활성 + tscn 6 노드 + demo 통합 + F6 sync + slot_loaded callback + R91/R90 회귀, **11/11 PASS**. R90/R91 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: D 90→**91%** (SaveListPanel 신규 10번째 UI 패널), E 80→**82%** (F6 워크플로우 통합), 종합 83.45→**83.93% (↑0.48%p)**, 출시 가능 기준 77.4→**77.6%**, Godot 93-96% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 91 종료) — **Save round-trip 정합성 fix (D+F 동시 개선)**. R90 spirit UI 정제 후속. **문제**: `apply_save` 가 class_id / stat_* / equipment / unlocked_skills / skill_levels / play_time_sec / gunner_* / active_* / quest / mission 복원 누락 → save → load 후 Sorcerer 가 Warrior 로 변하고 스탯/스킬/장비/퀘스트 lost (R86/R87 의 Sorcerer/spirit 작업이 round-trip 으로 무효화). **save_manager.gd::make_payload + game_state.gd::to_save_dict 양쪽에 11 누락 필드 추가** (stat_points, skill_levels, gunner_combo/max/ammo, active_curses/buffs/stances, mission). **game_state.gd::apply_save 대폭 보강**: class_id 복원 + stat_str/dex/int/con/points 5 복원 + equipment Array[int] 안전 재할당 + unlocked_skills 안전 재할당 + skill_levels JSON string-key → int 변환 + play_time_sec + gunner_* 3 + active_* 3 + Quest/Mission singleton from_save 호출. nested player + flat legacy fallback 유지 (R63 호환). `tools/h5_test_save_round_trip.py` 신규 (10 검증): to_save_dict 31 필드 / apply_save 18 복원 동작 / make_payload 17 필드 / Python JSON round-trip 시뮬 (src 31 필드 → JSON → restored 31 필드 모두 동일, class_id=4 Sorcerer 보존, skill_levels int key, equipment Array[int]) / nested+flat fallback / R90 회귀, **10/10 PASS**. R88-R90 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: D 카테고리 88→**90%** (Save/Load 88→95), F 카테고리 55→**60%** (Save round-trip 0→60 신규), 종합 82.47→**83.45% (↑0.98%p)**, 출시 가능 기준 76.8→**77.4%**, Godot 92-95→**93-96%**.

이전 라운드 업데이트: 2026-05-19 (Round 90 종료) — **Spirit/class skill desc placeholder + 줄바꿈 정제 (UI 표시 가독성)**. R88 의 EUC-KR 디코딩 후속. R88 raw desc 는 `}#NN<unit>|` placeholder + `;` 줄바꿈 마커를 entry["desc"] 에 그대로 저장. R90 은 `game_data` 에 **`resolve_skill_desc_display(class_id, skill_id) -> String`** (전체 desc) + **`resolve_skill_desc_first_line(class_id, skill_id) -> String`** (첫 segment) 2 helper 신규. (1) `resolve_skill_desc` 의 stat 치환을 위임받고, (2) `}TEXT|` 강조 브래킷을 `[TEXT]` 로 변환 (UI 가독성), (3) `;` → `\n` 변환. 짝 안 맞는 `}` 는 그대로 유지 (orphan-safe). 예 (spirit #0 암흑탄, stats[5]=120): raw `"거대한 암흑탄을 발사하여;정령마력 }#05%|의;피해를 준다.;..."` → display `"거대한 암흑탄을 발사하여\n정령마력 [120%]의\n피해를 준다.\n..."`. **battle_system SKILL action 정정**: R88 의 raw `desc_str.split(";")[0].strip_edges()` inline 코드 제거 → `GameData.resolve_skill_desc_first_line(5, skill_id)` 호출로 위임 (Sorcerer spirit 발동 시 placeholder 치환된 첫 줄 노출). spirit (class_5) stats_u16 의 실제 byte→field 매핑은 미확정 (R88 docstring 명시) 이므로 표시 숫자가 게임 원본과 일부 다를 수 있음 — 정확 매핑은 R91+ Formula::calc + spirit-specific layout RE. tools/h5_test_spirit_desc_placeholder.py 신규 (10 검증) — 시그니처 2 / 핵심 로직 marker 5 / battle_system delegation / Python 시뮬 (`}#05%|` + stats=120 → `[120%]`, 듀얼 placeholder, orphan `}`, first_line 분리) / R87-R89 회귀, **10/10 PASS**. R88 회귀 테스트 도 R90 helper 인식하도록 정정 (R88 inline 또는 R90 GameData helper 어느 한쪽 허용). R82-R89 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: F 카테고리 50→**55%** (Spirit desc placeholder resolution 0→90 신규, UI 노출 정제로 Sorcerer 사용성 +5%p), 종합 81.97→**82.47% (↑0.50%p)**, Godot 92-95% 유지, 출시 76.8% 유지 (F 는 출시 가중치 외).

이전 라운드 업데이트: 2026-05-19 (Round 89 종료) — **Android launcher icons asset 생성 (PIL 기반)**. R86 의 `launcher_icons` 3 슬롯 (`main_192x192` + `adaptive_foreground_432x432` + `adaptive_background_432x432`) 빈 칸을 채움. tools/h5_make_launcher_icons.py 신규 (147 줄, PIL + 3 helper + sword/star drawing + 5-platform font fallback). icon.svg 디자인 (검+별+"HERO 5") 재현. 3 PNG 생성: main_192×192 opaque (2844B) / adaptive_foreground_432×432 transparent BG (2297B) / adaptive_background_432×432 (3238B). export_presets.cfg Debug + Release 모두 `res://assets/launcher_icons/*.png` 참조 (이전 빈 칸 → 정확 경로). tools/h5_test_launcher_icons.py 10/10 PASS, R82-R88 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: G 카테고리 55→**65%** (launcher_icons 0→90 신규), 종합 81.17→**81.97% (↑0.80%p)**, 출시 가능 기준 76→**76.8%**. Godot 92-95% 유지.

이전 라운드 업데이트: 2026-05-19 (Round 88 종료) — **Spirit desc EUC-KR 디코딩 (16/16 한국어 텍스트 추출)**. R87 explicit field 추출 후속, R77 file layout 의 desc_string 영역 (bytes[48..48+desc_len], EUC-KR 인코딩) 후처리. **tools/converter/decode_h5_skill_desc.py 신규**: c_csv_skill_05.json 의 16/16 record 에 `desc_text` 필드 in-place 추가 (평균 68.9 글자). 샘플: spirit #0 암흑탄 = "거대한 암흑탄을 발사하여;정령마력 }#05%|의;피해를 준다.;마법공격력으로;위력이 증가." / #7 정신감응 = "패시브 스킬.;전투시 정령 게이지가;충전되는 양이;}1.5배| 증가한다." `;` = 줄바꿈. game_data._ensure_spirit_skills_loaded() 가 desc_text → entry["desc"] 채움. resolve_skill_desc class_5 분기에 자동 spirit loader 호출. battle_system._skill_data 가 desc 반환 + SKILL action 발동 시 Sorcerer 의 spirit 사용시 첫 라인 로그 노출 (`  ▸ ...`). tools/h5_test_spirit_desc_decode.py 13/13 PASS, R75-R87 회귀 모두 통과. verify_godot_project **0 warnings**. **진척률 갱신**: F 카테고리 40→50%, 종합 80.17→**81.17% (↑1.00%p)**, Godot 92-95% 유지, 출시 82% 유지 (F 는 출시 가중치 외).

이전 라운드 업데이트: 2026-05-19 (Round 87 종료) — **Spirit (class_5) extra_hex full 의미 매핑 — 80% 돌파**. R83/R84 후속 최종화. game_data._ensure_spirit_skills_loaded() 가 R77 정확 sub-rel offset 으로 8 explicit field 추출 (effect_type 0x1a, dynamic_formula 0x26, special_dispatch 0x2b, formula_id_1/2 0x2d/0x2e, primary_u16 0x22 LE, secondary_u16 0x24 LE, desc_len 0x2f). skill_info(5,_) 가 explicit field 직접 반환. Spirit 분포 effect_type 0=5/2=9/7=2 (debuff 위주), formula_id_1=57 (10/16). Sorcerer spirit fallback 정확화. tools/h5_test_spirit_full_mapping.py 9/9 PASS, R82-R86 회귀 통과. F 27→40%, 종합 78.87→80.17% (↑1.30%p) — **80% 돌파**. Godot 92-95%, 출시 82%.
