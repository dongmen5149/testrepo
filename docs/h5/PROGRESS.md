...
업데이트: 2026-05-06 — Phase 2-A 완료 (sprite 3,798 frame + 한글 텍스트 75,284개 추출 / 35,931 unique).
...
**한글 텍스트 일괄 추출** (588 파일에서 EUC-KR 추출):
- **75,284 string occurrences / 35,931 unique strings**
- 검증 어휘: "퀘스트"(415), "있습니다"(416), "하지만"(211), "제한레벨"(207), "스킬북"(200), "아이템"(170), "피해를"(129)…
- 한자 영역 false-positive 다수 (`拙`/`葺`/`羲` 반복) — 0xb0a1–0xc8fe 한글 영역 한정 필터 필요 (다음 작업)
...
#### 2-A.6 다음 단계 (Phase 2 마무리)

1. **한글 추출 필터 개선** — EUC-KR 한글 정확 영역(0xb0a1–0xc8fe) + 한자 영역(0xca–0xfd) 분리, false-positive 제거.
2. **TINY_META (104 파일) 구조 분석** — 5–50B 의 sprite 부속 메타 (hitbox/offset). signed int16 LE 패턴으로 파싱.
3. **MID_SCRIPT/LARGE_ANIM 의 19 19 마커 의미** — 청크 분리자/프레임 표지자/end-of-record 후보. opcode 디스어셈블 시도.
4. **Ghidra 분석** — `loadAssetFromVFS` 디스패치 테이블 + `getUniqueAssetNameFromID` 로 자산 이름 복원.
