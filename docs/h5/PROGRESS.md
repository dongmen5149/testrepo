...
업데이트: 2026-05-06 — Phase 1 완료 + Phase 2-A.1~4 완료 (sprite 디코더 — 유효 frame 100% 렌더링 / 3,716개).
...
- 일괄 디코딩 결과: 409 파일 / 4,186 frame / **3,716 렌더링 (88.8%, 유효 sprite 100%)** / 0 에러
...
1. **소수 클러스터 분류** — `01640201`(27), `02000100`(21), `01000100`(43), `01010101`(14) 등 50+ distinct magic 의 작은 파일들 = 애니메이션 스크립트 / 폰트 / 데이터 테이블 후보. 첫 4 byte 가 매우 다양함.
2. **embedded _pa 와 별도 _pa 의 관계** — sprite frame 내 팔레트와 _pa 557 파일의 대응 매핑. hash 인접성·인덱스 분석.
3. **자산 이름 복원** — `AndroidService::getUniqueAssetNameFromID` 디컴파일 → 2,189 파일에 의미있는 이름 부여.
4. **애니메이션 메타** — sprite 파일의 frame 순서/타이밍 정보가 sprite 내부에 있는지 별도 파일인지 확인.
