...
#### 2-A.7 다음 단계 (Phase 2 마무리)

1. **TINY_META 정식 파서** — 7-byte record 의미 확정 (Ghidra `loadFrameMeta` 류 함수 추적).
2. **MID_SCRIPT/LARGE_ANIM 의 19 19 마커 의미** — 청크 분리자/프레임 표지자/end-of-record 후보.
3. **자산 이름 복원** — Ghidra `AndroidService::getUniqueAssetNameFromID` 디컴파일 → 2,189 파일에 의미있는 이름.
4. **Ghidra `loadAssetFromVFS` 디스패치 테이블** — `(asset_id) → reader_fn` 매핑으로 잔여 OTHER 508 / LARGE_RAW 288 분류.
