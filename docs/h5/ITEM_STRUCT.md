...
| 17 | 0x138 (312B) | 0xa47c0 (SkillBook, case 16/17 공유) | +0x134/0x135/0x136/0x137 (4 byte) — Round 20 |
...
- `parse_skill_book_extra` (slot_17, Round 20): val_134/val_135/skill_level/val_137 (4 byte)
- `parse_cash_extra` (slot_18, Round 20): val_134/val_135 (2 byte)
- mix (cat 14/15) 만 추가 fields 없음

## Round 20 (2026-05-10) — slot_17/slot_18 layout 정확 식별

### slot_17 (SkillBookItem) — 4 byte ext @ 0xa47c0
...
| offset | 의미 | 검증 |
|---:|---|---|
| `+0x134` | u8 skill_class | slot_17 records 모두 v134 ∈ {2, 3} — 2 = Gunslinger 계열, 3 = Knight 계열 |
| `+0x135` | u8 skill_id (within class) | v135 ∈ 0..9 — 클래스별 10 개 스킬 |
| `+0x136` | u8 skill_level | "연속사격LV1..LV4" → v136 = 1, 2, 3, 4 정확 매칭 ✓ |
| `+0x137` | u8 required_level | LV1..4 = 1, 4, 10, 22 monotonic increasing |

검증 records: 연속사격LV1..LV7, 동시사격LV1..LV4, 샷건LV1..LV5, 화염방사LV1..LV7,
충격포LV1..LV5 등 총 98 records.
