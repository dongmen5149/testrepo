...
## Round 25 (2026-05-10) — slot_15 (mix_book recipe) 13 byte ext 구조 RE

### 13 byte recipe layout (LoadItemTable case 15 @ 0xa4578)

| offset | 크기 | 의미 |
|---:|---:|---|
| `+0` | 1B | 0x00 (separator/version flag) |
| `+1..+3` | 3B | **ingredient 1**: (cat, idx, count) — 0xff sentinel = unused |
| `+4..+6` | 3B | **ingredient 2**: same format |
| `+7..+9` | 3B | **ingredient 3**: same format |
| `+10..+11` | 2B | **result**: (cat, idx) — count = 1 |
| `+12` | 1B | **success_rate %** (0x64=100, 0x5a=90, etc.) |

cat 인덱스는 items.json 의 slot_NN 과 동일:
- `0x00..0x09` = EquipItem (slot_0..9 = weapon/armor/etc, 무기 제작 결과)
- `0x0b` (=11) = slot_11 (potion)
- `0x0d` (=13) = slot_13 (mix material — 살코기/약초 등 raw)
- `0x0e` (=14) = slot_14 (mix material_2 — 황혼수프가루 등 cooked)

### Recipe 패턴 (items.json 116 records 검증)

| 종류 | 결과 cat | 예시 | success rate |
|---|---:|---|---:|
| 쿠킹 (mix material → cooked food) | 0x0e | 살코기 ×5 + 황혼버섯 ×5 → **황혼수프가루** | 100% |
| 포션 합성 (potion → upgraded) | 0x0b | 포션 ×2 → **미들포션** | 100% |
| 퀵포션 (potion + herb → quick) | 0x0b | 포션 ×2 + 지혈초 ×1 → **퀵포션** | 100% |
| 재료 정제 (raw → refined) | 0x0d | 엑토플라즘 ×10 → **에테르** | 90% |
| 일반 무기 제작 | 0x00..03 | 보통 칼날 ×20 + 강화 가죽 ×6 + 강철 ×3 → **투란기어** | 90% |
| 중급 무기 제작 | 0x05..09 | various | 60-70% |
| Legendary 무기 제작 | 0x00..09 | various 고급 재료 | 20-22% |

success_rate 분포:
- 100% (6 records): basic potion 합성
- 90% (20 records): 재료 정제, 일반 무기 제작
- 60-70% (~25 records): 중급 무기 제작
- 20-22% (~25 records): legendary 무기 제작 — 게임 밸런스 (높은 등급일수록 낮은 확률)

### decode_h5_item.py 새 fields (Round 25)

`parse_mix_book_extra` 가 `recipe` 객체 부여:
```json
{
  "recipe": {
    "ing1": {"cat": 13, "idx": 0, "count": 5},   // 살코기 ×5
    "ing2": {"cat": 13, "idx": 1, "count": 5},   // 황혼버섯 ×5
    "ing3": null,                                // 미사용
    "result_cat": 14, "result_idx": 1,           // → 황혼수프가루
    "success_rate": 100
  }
}
```
