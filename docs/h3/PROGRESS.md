...
**최신 커밋 시점**: 2026-05-10 PM-2 — **2F + 2G 일괄 + Ghidra PIC 실패 패턴 발견**. (1) ⭐ mode 2 (FUN_00060ab4) 가 1.5KB+7KB 가 아니라 **8.6KB 100% 코드** 로 정정 — 이전 capstone stop 은 단순 alignment 이슈. (2) FUN_00064048 (default key handler) 분석 — **state machine 아님**, ring buffer 직렬화 호출 패턴. (3) `state[0x460]` 정체 = **menu item 선택 highlight 플래그** (battle 트리거 아님). (4) ⭐⭐ **402 PIC 함수 (~27%) Ghidra 디컴파일 실패** 패턴 발견 — 지금까지 자동 분석 ceiling 의 진짜 원인. 신규 도구 2개 (`parse_mode2_ui_data.py`, `disasm_default_key_handler.py`) — `walk_with_skip` 패턴 표준화. 상세는 [ghidra-mode2-default-handler-2026-05-10.md](ghidra-mode2-default-handler-2026-05-10.md).

**이전 세션** (2026-05-10 AM): `state[0x94]` 재해석 + 3 entry caller 한계 확정. `FUN_00070f34` (key handler) 발견. 상세는 [ghidra-scn-dispatcher-2026-05-10.md](ghidra-scn-dispatcher-2026-05-10.md).

### 한 줄 요약 (현재 상태)

영웅서기3는 **1주차 콘텐츠 완성도 높은 플레이 가능 게임** + **§4.4 NPC behavior dispatcher 95% 해독** + **state[0x94]/state[0x460]/mode 2 모두 정정** + **402 PIC 함수 Ghidra 실패 패턴 식별** (2026-05-10 PM-2). `walk_with_skip` 표준 도구 확립으로 다른 stub 함수 분석도 동일 패턴 가능. 다음 진척은 **(1) 402 stub 함수 우선순위 분류, (2) FUN_0007e150 큐 producer/consumer 매핑, (3) SMAF/번역 사용자 블로커**.
...
4. **[ghidra-mode2-default-handler-2026-05-10.md](ghidra-mode2-default-handler-2026-05-10.md)** ⭐⭐⭐ — **최신 PM-2** (mode 2 정정 + default key handler + 402 stub 패턴)
5. (참고) [ghidra-scn-dispatcher-2026-05-10.md](ghidra-scn-dispatcher-2026-05-10.md) — AM 의 state[0x94] 재해석
6. (참고) [ghidra-scn-dispatcher-2026-05-09c.md](ghidra-scn-dispatcher-2026-05-09c.md) — 그 이전 자동 분석 종합
7. (선택) 빌드 검증 — 아래 §"재현 명령"
...
| ⭐ **2I** | **402 stub 함수 우선순위 분류 + 분석** | Ghidra 가 `{ FUN_0004ad10(); }` 단일 stub 으로 디컴파일한 PIC 함수 402개 (1470 중 ~27%). raw bytes / 다음 함수 entry 거리 / BL 호출 빈도 등으로 ranking → top 후보부터 capstone walk_with_skip 으로 분석. 자동 가능 |
| ⭐ **2J** | **FUN_0007e150 큐 producer/consumer 매핑** | 이번에 발견된 GVM 이벤트 큐 API. 누가 큐에 쓰고 누가 dequeue 하는지 → 게임 로직 진입점 후보. all_decompiled.c grep + capstone caller 추적 |
...
- [ghidra-mode2-default-handler-2026-05-10.md](ghidra-mode2-default-handler-2026-05-10.md) — **⭐⭐⭐ 최신 PM-2** (mode 2 정정 + default key handler + 402 stub 패턴)
