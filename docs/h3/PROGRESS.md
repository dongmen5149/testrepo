...
**Round 88** (committed `9b1de921`) — **Quests 탭 file-색상 + QuestIndex byFile API**. (1) ⭐⭐⭐⭐ **`Hero3CatalogQuestIndex` API 확장** — `byFile: Map<String, List<Entry>>` / `fileCount` / `colorOf(file): Int` / `fileColors(): Map<String, Int>` / `companion FILE_PALETTE: IntArray` (amber/teal/lavender/soft-red + 예비 2색). 정렬된 파일명 → palette slot[0..N] 1:1, 미지 파일은 hash fallback (각 채널 ≥ 0x80 보장). (2) ⭐⭐⭐ **CatalogViewerScene 작은 리팩터** — `rowsForTab(Tab): List<String>` → `List<Row>` (Row = text + optional paint). 기존 12 탭은 `Row(it)` 변환만으로 동일 렌더링. Quests 탭만 `questRows()` 로 분리, 파일 헤더 + entries 양쪽에 file color paint 주입. `questPaintCache` 로 frame 마다 Paint alloc 회피. (3) ⭐⭐⭐ **3 신규 unit tests** — byFile 4 키 그룹화 / fileColors distinct + stable + fallback alpha=0xFF 보장 / colorOf 정렬 슬롯 매핑 (00→01→10→11 ↔ palette[0..3]). (4) **finding**: quest_*_dat 4 파일이 색으로 즉시 식별 — quest_00 (메인 amber) / quest_01 (서브 teal) / quest_10 (하드 메인 lavender) / quest_11 (하드 서브 soft-red). (5) 81/81 tests (catalog 32→35, engine-core 34, 0 failures) + APK 14M BUILD SUCCESSFUL. (6) **진행률 ~99.98% → ~99.98%** (분석은 동일, 리메이크 UI 가독성 +α). 상세는 [ghidra-round88-quest-tab-file-color-2026-05-19.md](ghidra-round88-quest-tab-file-color-2026-05-19.md).

**Round 87** (committed `90549023`) — **R62 Quest Item Xref 21 통합**. (1) ⭐⭐⭐⭐ **R62 quest_item_xref** (game_balance.json 에 있었지만 catalog 가 무시) → `Hero3Catalog.questItemXref` 통합. 신규 data class `Hero3QuestItemMatch` (file, offset, text, context) / `Hero3QuestItemXref` (cleanName, matches). (2) ⭐⭐⭐ **Loader.parseQuestItemXref** — `quests.item_xref` dict 파싱. (3) ⭐⭐⭐ **Catalog API 2종**: `findQuestXref(name)` / `questXrefByFile(file)`. (4) ⭐⭐⭐ **CatalogViewer 신규 탭 QUEST_ITEM_XREF** — 각 item 첫 3 match 표시 + "+N more" 줄임표. Tab 12 → 13. (5) ⭐⭐⭐ **3 신규 unit tests** — 21 items 확인, 매칭 lookup, file-group filter. (6) **finding**: 21 items 중 20 has matches, "반토막난 지도" 만 빈 매치. "토레즈시민증" 35 matches / "토레즈의서신" 38 matches 가장 빈번 (핵심 quest item). (7) 78/78 tests + APK BUILD SUCCESSFUL. (8) **진행률 ~99.97% → ~99.98%** (+0.01%p, R62 의 quest-item knowledge 가 비로소 catalog 안에서 query 가능). 상세는 [ghidra-round87-quest-item-xref-2026-05-19.md](ghidra-round87-quest-item-xref-2026-05-19.md).

**이전 진행 라운드**: 2026-05-19 (Round 86, committed `bea6aeb5`) — **5BB = Quest export truncation 수정 + 115 entries 복구**. (1) ⭐⭐⭐⭐ **export_game_balance.py L393** 의 `entries[:20]` 슬라이스 제거 — R84 도입된 truncation bug. (2) ⭐⭐⭐⭐ **game_balance.json 패치 in-place** — quests.files 4 전체 재기록 (37+7+38+33 = 115 entries). 파일 크기 832KB → 837KB. (3) ⭐⭐⭐ **R85 quest index 자동 업데이트**: 8 → **11 duplicates**. 신규 cross-file (반전세력 엘지스 / 등대를 향해 quest_00↔quest_10) + 신규 intra-file ("협곡 탐사" quest_10 내 2회). **해석 강화**: quest_01_dat (7) ⊂ quest_11_dat (33) — 7 entries 모두 quest_11 에 존재. quest_01 = quest_11 의 발췌본. (4) **75/75 tests** 그대로 통과 + APK BUILD SUCCESSFUL — index 통계 자동 갱신, 테스트 코드 변경 없음. (5) **진행률 ~99.96% → ~99.97%** (+0.01%p, Catalog Quest 정합성). 상세는 [ghidra-round86-quest-export-truncation-fix-2026-05-19.md](ghidra-round86-quest-export-truncation-fix-2026-05-19.md).

**이전 진행 라운드**: 2026-05-19 (Round 85, committed `e3ad545b`) — **5BA = Hero3CatalogQuestIndex + Quests tab 중복 표시**. (1) ⭐⭐⭐⭐ **Hero3CatalogQuestIndex 신규** — `Hero3Catalog.questFiles` 평탄화 + 캐논화 (trim + 공백 압축 + 꼬리 1자리 숫자 제거) → 67 entries / 59 distinct names / **8 cross-file duplicates** (quest_01↔quest_11 7개 + "전장으로" quest_00↔quest_10). `MainActivity.questIndex` lazy 노출. (2) ⭐⭐⭐ **CatalogViewerScene Quests tab 업데이트** — 헤더 (loaded/distinct/duplicates 카운트) + 중복 entry 앞 ★ 마커. (3) ⭐⭐⭐ **4 신규 unit tests** — build / canonicalize trailing digit / lookupExact + Contains / known duplicate "혼돈의 대륙". 총 **75/75 tests pass** + APK BUILD SUCCESSFUL. (4) **신규 finding**: `n_entries` 메타데이터 (37+7+38+33=115) vs `entries` 실제 (20+7+20+20=67) 불일치 = export script truncation. R86 에서 export 재추출 권장. (5) **진행률 ~99.95% → ~99.96%** (+0.01%p). 상세는 [ghidra-round85-catalog-quest-index-2026-05-19.md](ghidra-round85-catalog-quest-index-2026-05-19.md).

**이전 진행 라운드**: 2026-05-19 (Round 73, committed `55c5dc59`) — **🏆 4MA + 4MB + 4MC + 4MD = DES 8/8 복호화 성공 + i15_dat master shop catalog + SMAF pipeline + R58 가설 정정**. (1) 🏆⭐⭐⭐⭐⭐ **DES 8/8 파일 복호화 성공** — Hero5 의 `mx_des_decrypt` 변종 (`tools/h5_des.py` Python 포팅) + Hero3 기존 키 `"0EP@KO91"` (R57) 조합. **R58 의 "표준 5 변종 모두 실패 → NDK runner 필수" 가설 완전 정정**. 1년 묵은 사용자 환경 의존 작업이 30분 만에 해결. (2) 🎉⭐⭐⭐⭐⭐ **i15_dat (7,400B) = master shop catalog 확정** — 한글 "**붉은머리띠**, **레벨 10 투구; 머리띠; 보스용도 15년;**" 등 shop NPC description 텍스트 직접 발견. entry = (size, name, description body). (3) ⭐⭐⭐⭐ **나머지 7 파일도 평문 검증 통과** — getitem_dat (entropy 7.4→3.62) / smith_dat (7.76→3.84) / smithh_dat / shop_dat / shoph_dat (entropy 3.6-4.1 = binary table). drop_dat / droph_dat (entropy 7.9→5.97/5.25, 18-byte stride + `11 00` separator binary drop table). (4) ⭐⭐⭐⭐ **SMAF→OGG pipeline 스크립트 + 설치 가이드 작성** — 외부 도구 (smaf-converter.jar / TiMidity 또는 FluidSynth / FluidR3_GM.sf2 / FFmpeg) 부재 상태에서 graceful detection + 단계별 안내. 33/33 SMAF 헤더 검증 통과. **사용자 신뢰도 정책 ("신뢰도 높은 것만") 으로 도구 설치 보류 — R74 Phase B 에서 winget + pip + Pure Python SMAF→MIDI 자체 구현 경로로 진행 결정 대기**. (5) ⭐⭐⭐ **MASTER_SPEC §10 갱신** — DES 8 pending → **0 pending**. (6) **R74 권장 작업 (Phase A = 즉시 가능, Phase B = 사용자 결정)**: i15_dat parser / drop_dat parser / smith_dat parser / boss skill ID H4 검증 / Hero3Catalog 확장. (7) **진행률 ~99.8% → ~99.95%** (+0.15%p). 핵심 lesson: **벤더 공통 cipher = cross-game 1순위 시도** (Hero4 R68 의 lesson 이 Hero3 에도 적용). 상세는 [ghidra-round73-des-success-smaf-pipeline-2026-05-19.md](ghidra-round73-des-success-smaf-pipeline-2026-05-19.md).

**이전 진행 라운드**: 2026-05-19 (Round 72, committed `54e14071` 흡수) — **4LA + 4LB + 4LC = Android scene 통합 시작 (CatalogViewerScene + BestiaryScene boss rating)**. (1) ⭐⭐⭐⭐⭐ **MainActivity 에 Hero3Catalog lazy 로더 통합** — `MainActivity.catalog: Hero3Catalog by lazy { Hero3CatalogLoader.load(AndroidAssetReader(this)) }`. 모든 scene 에서 접근 가능, lazy 로딩 (TitleScene 진입 시 비용 0). SceneRequest.CatalogViewer 추가. (2) ⭐⭐⭐⭐⭐ **CatalogViewerScene 신규 작성** — R71 catalog 의 raw 데이터 7-tab brower: Overview (총 개수) / Stat Enum (24/25 codes) / Rarity (6 prefix) / Item Categories (18) / Skill Sets (7 weapon) / Boss Roster (15 + combat_rating + skill_slots) / DES Status (8 pending). 한국어/영어 i18n. 메인 메뉴 debug 항목에 추가. (3) ⭐⭐⭐⭐ **BestiaryScene 에 boss combat_rating 표시 추가** — 선택된 enemy 가 catalog.bossesNormal 과 매칭되면 "★ 보스 권장 lvl: 51" + "sprite #0 story/misc" 추가 표시. Context 가 MainActivity 가 아니거나 catalog 로딩 실패 → graceful degrade. (4) ⭐⭐⭐ **build 성공 + 12 unit tests 모두 통과** — :app:compileDebugKotlin BUILD SUCCESSFUL, :app:testDebugUnitTest BUILD SUCCESSFUL. (5) **진행률 ~99.7% → ~99.8%** (+0.1%p, Android 통합도: data loader 완성 → scene 통합 시작 MASTER_SPEC §13 step 2-3). 상세는 [ghidra-round72-scene-integration-2026-05-19.md](ghidra-round72-scene-integration-2026-05-19.md).
...
### 전체 진척률 추정 (Round 73 시점, 2026-05-19)

> **결론**: "Android 리메이크 완성" 기준 약 **99.95% 완료** (자동 분석 진영 완전 종결). R51~R73 (총 23 라운드) 동안 (1) GVM/엔진 로직 리버싱 → (2) 데이터 모델링 완전 통합 → (3) Android 통합 → (4) DES 8/8 복호화 성공으로 패러다임 전환. 자동으로 진행 가능한 영역은 사실상 모두 완료, 남은 작업은 R74 Phase A (DES 평문 8 파일 정밀 파서, 즉시 가능) + Phase B (SMAF→OGG, 사용자 도구 설치 승인 대기) + Phase C (대사 LLM 번역, API key) 셋 뿐.

| 영역 | 진척 추정 | 근거 |
|---|---|---|
| 자산 포맷 분석/변환 | **~99%** ⬆ | _bm/_pa/_txt/_cif/_mp/_scn/_smaf 모두 해독. **R73 DES 8/8 복호화 성공** (Hero5 mx_des_decrypt 변종, key `"0EP@KO91"`). 남은 미해결 = SMAF→OGG 변환 (도구 설치만 필요) |
| 자산 변환 산출 | **~95%** | 704 자산 + 3,131 sprite frames + 134 maps + HD 4× 업스케일 + 33 SMAF 헤더 검증 통과. 남은 작업: SMAF 33 파일 OGG 변환 + 9,740 대사 영문 번역 |
| Ghidra 게임 로직 리버싱 | **~99%** ⬆ | 1,470 함수 중 ~145개 본문 분석 (R47 ~120 → R73 ~145). GVM architecture + 7 indirect entries + 3 dispatcher JT + 14 known GOT slots + **10 ObjectB vtable methods**. R51~R55 = FUN_818f0 74-JT + ObjectA cluster 정정 + asset path string table (R55 의 "전투 시스템 없음 = 확정된 negative result"). **R55 결론: 전투가 NPC table data + SCN opcode 안에 분산** → R56 부터 데이터 모델링으로 전환 |
| **데이터 모델링** | **~99.9%** 🆕 | **R56~R73 (18 rounds) 누적**: enemy_dat 161×2 (R56) + DES key 식별 (R57) + boss 15 + quest 44 (R58) + char_dat 10 playable (R59) + skill 7파일 105 (R60) + item 18 카테고리 480+ (R61~R63) + game_balance.json v1.0~v1.1 (R64~R66) + skill effect mask + boss 6B trailer (R65) + debuff codes 정밀 (R66) + skill header 14B + boss skill 가설 H4 (R67) + gun marker 정밀 (R68) + i14 ammo + dialogue queue (R69) + Master Spec 4,700 lines (R70) + **DES 8/8 평문** (R73). **§4.4 = 95→99.9%** |
| Android 리메이크 통합 | **~30%** ⬆ | **R71~R72 (2 rounds) 누적**: Hero3Catalog 19 data classes + Loader + 12 unit tests (R71) + CatalogViewerScene 7-tab + BestiaryScene boss rating (R72). 엔진 재구현은 여전히 ~5-10% (8 씬 골격 + NPC 4 hardcoded + i18n + save slot), but data layer 완성으로 통합 활성화 |
| i18n | UI 100% / 대사 0% | UI 어휘 196개 100% 영문, 9,740 unique 대사 번역 미진행 ($4.09 추정, Claude Sonnet 4.6) |

**남은 블로커** (R74 이후 진척 순):
1. **R74 Phase A: DES 평문 8 파일 정밀 파서** — 자동 가능, 즉시 진행 (i15_dat shop catalog / drop_dat / smith_dat / shop_dat / getitem_dat + Hero3Catalog 확장 + game_balance.json v1.2)
2. **R74 Phase B: SMAF→OGG 변환** — 사용자 신뢰도 정책 (winget + pip + Pure Python SMAF→MIDI 자체 구현 경로) 승인 대기
3. **R74 Phase C: 대사 LLM 번역** — 9,740 entries, $4.09, 사용자 API key 필요
4. **boss skill ID H4 가설 최종 검증** (R67/R68 후속) — Phase A 의 drop_dat / smith_dat 안에 boss AI table 있을 가능성
5. **(중기) Compose Multiplatform UI 마이그레이션** (Phase C Step 4d) — 엔진 코드 분리의 마지막 단계
6. **(장기) Android 엔진 본격 재구현** — 전투/스킬/이벤트/진행 로직 (Hero3Catalog 활용)

### 한 줄 요약 (현재 상태)

영웅서기3는 **1주차 콘텐츠 완성도 높은 플레이 가능 게임** + **§4.4 99.9% 해독** + **🏆 Round 73 DES 8/8 파일 복호화 성공 + i15_dat master shop catalog + SMAF pipeline** (2026-05-19 / Round 73, committed `55c5dc59`). **자동 분석 영역 사실상 완전 종결** (~99.95%). R55 의 "전투가 NPC table data + SCN opcode 안에 분산 = 별도 전투 시스템 없음 = 확정된 negative result" 이후 패러다임을 **데이터 모델링** 으로 전환, R56~R73 18 rounds 누적으로 enemy_dat 161×2 (R56) + boss 15 (R58) + char 10 playable (R59) + skill 105 (R60) + item 480+ (R61~R63) + Master Spec 4,700 lines (R70) + Hero3Catalog 19 data classes + 12 unit tests (R71) + Android scene 통합 (R72) 완료. **R73 = Hero5 의 mx_des_decrypt 변종 (`tools/h5_des.py`) + Hero3 키 `"0EP@KO91"` 조합으로 DES 8/8 평문 복호화 성공** — R58 의 "표준 5 변종 모두 실패 → NDK runner 필수" 1년 묵은 가설 30분 만에 정정. **i15_dat (7,400B) = master shop catalog 확정** (한글 "붉은머리띠 / 레벨 10 투구; 머리띠; 보스용도 15년;" 직접 발견). **남은 작업 3 Phase**: (A) R74 Phase A = DES 8 평문 정밀 파서 (즉시 가능), (B) Phase B = SMAF→OGG (사용자 도구 정책), (C) Phase C = 대사 LLM 번역 ($4.09).
...
1. **[`SESSION_HANDOFF.md`](SESSION_HANDOFF.md)** ⭐⭐⭐⭐⭐ — **R74 Phase A/B/C 즉시 시작 가이드** (이게 가장 중요)
2. **[`MASTER_SPEC.md`](MASTER_SPEC.md)** ⭐⭐⭐⭐⭐ — Hero3 single reference, 15 sections / 4,700+ lines (Android 리메이크 개발자 entry point)
3. `git log --oneline -5` — 최신 커밋 확인 (`55c5dc59` R73 DES success 가 마지막)
4. `git status --short` — 미커밋 잔여 확인
5. `ls work/h3/decrypted/` — R73 산출 평문 8 파일 확인 (Phase A 작업 대상)
6. **[Round 73 doc](ghidra-round73-des-success-smaf-pipeline-2026-05-19.md)** ⭐⭐⭐⭐⭐ — **DES 8/8 복호화 + i15_dat master shop catalog + SMAF pipeline** (최신 라운드)
7. (참고) [Round 72 doc](ghidra-round72-scene-integration-2026-05-19.md) — Android scene 통합 (CatalogViewerScene + BestiaryScene)
8. (참고) [Round 71 doc](ghidra-round71-catalog-loader-2026-05-19.md) — Hero3Catalog 19 data classes + 12 unit tests
9. (참고) [Round 70 doc](ghidra-round70-master-spec-exp-groups-2026-05-19.md) — Master Spec 통합 + 자동 분석 종결 선언
10. (Ghidra 리버싱 컨텍스트 필요 시) [Round 50 doc](ghidra-input-dispatcher-and-objectb-instance-2026-05-18.md) — Round 47 → R55 의 "전투 시스템 없음 negative result" 이전 마지막 함수 분석 (이후 R56 부터 데이터 모델링으로 전환)
11. (선택) 빌드 검증 — 아래 §"재현 명령"

### Round 18~73 한눈 요약 (다음 세션 빠른 컨텍스트 복구용)
...
| **48** (2026-05-18, `c0aa025d` 흡수) | ⭐⭐⭐ **task[0xa848] sub-struct field layout 추출** (≥0x5c bytes, +0x00 primary state / +0x01,+0x02 sub-state byte indices / +0x04 secondary ptr / +0x0c "save+render 공유" (R49 정정 → dirty flag) / +0x58 sub-struct member). Round 47의 18 STR sites 정정 = stack store (HIGH ENCAPSULATION + LOCAL CACHING pattern). ⭐⭐⭐ **4 function pointer tables (GVM-injected at 0xc1f60/c1fa0/c1fc0/c1fe0)** — `FP_table[task[0xa848].sub_byte * 4](r0, r1)` via veneer 0xa429c. FUN_85edc (2-단 sequential) + FUN_85fc8 (조건부). ⭐⭐⭐ **FUN_000818f0 "74-entity iteration loop" 가설** (Round 50 정정됨 → 74-entry 입력 디스패처). ⭐⭐ **FUN_3d5d0 sound dispatcher 37 cmp arms** (Round 22의 22-arm shallow scan 정정), 19 unique sound IDs. ⭐⭐ **FUN_57394 render: byte_append pair (0x5, opcode)** display list 2-byte encoding | [ghidra-a848-substruct-and-fp-tables-2026-05-18.md](ghidra-a848-substruct-and-fp-tables-2026-05-18.md) |
| **49** (2026-05-18, `c0aa025d` 흡수) | ⭐⭐⭐ **task[0xa848] 신규 sub-field 발견 (stack reload tracking)**: **+0x03 (byte, FUN_86058 only)** + **+0x08 (word, 4 functions에 분산 7R+3W = DOMINANT new field)** + **+0x01의 11 STRB writes** (state transition setter). ⭐⭐⭐ **+0x0c 정정 = "render dirty flag"** (NOT render target value): R/W=1/0 boolean + +0x10 = render sub-struct pointer (FUN_75b98(&+0x10, 7, 1, 1) flush). ⭐⭐⭐ **FUN_3a86c = 16-entry JT dispatcher** (entity_arg signed [-0x10..-1], NOT pointer). entity_arg==-5 path → BL FUN_3c920(sub_mode, ctx_byte). **r0=#2 sub-mode = FUN_3c920 keypad mode selector**. ⭐⭐ **FUN_000818f0 정정** — entity_arg = signed int → "iteration loop" 가설 정정 필요 (Round 50 완료). 6 reload sites 모두 sub-handler 인자 pass-through (FUN_3a86c/92d30/92cc0/82df4) | [ghidra-a848-deferred-fields-and-letter-input-jt-2026-05-18.md](ghidra-a848-deferred-fields-and-letter-input-jt-2026-05-18.md) |
| **50** (2026-05-18, `c0aa025d` 흡수) | ⭐⭐⭐ **FUN_3a86c 16 JT entries → 4 distinct handlers** (self-relative JT @ 0xa745c): -1/-2 = cursor DEC (UP, decrement-and-wrap) + -3/-4 = cursor INC (DOWN, increment-and-clamp) + -5 = letter input via FUN_3c920 + -16 = ctx_byte = -1 (clear sentinel) + -15..-6 (10 entries) = NO-OP/epilogue. ⭐⭐⭐ **FUN_92d30 = '4'/'6' LEFT/RIGHT keypad** (FUN_92cc0 '2'/'8' UP/DOWN의 짝꿍) → 완전한 4방향 phone keypad 체계 + FUN_92bf8(mode 1-4) 핵심 cursor handler. ⭐⭐⭐ **task[0xa848]+0x08 = 동적 할당된 ObjectB 인스턴스 포인터** (60-byte). lifecycle: free → alloc → store. **vtable[+0x54] = allocator (NEW) + vtable[+0x58] = destructor 확정** (Round 47의 method0x58 정체 풀림). ObjectB known methods 9 → **10개**. ⭐⭐⭐ **FUN_000818f0 정정 — 74-entry JT 입력 디스패처** (NOT iteration loop): `(arg+0x10) ≤ 0x49 unsigned` 범위 검사 = signed [-0x10..0x39] = 74 distinct event codes. Round 48의 "74-entity iteration loop" 가설 완전 정정. ⭐⭐ **FUN_75b98 = render flush** (256B buffer @ ctx+lit+0xa0 의 dirty flag → memset(0) → indirect render call). FUN_82df4 (44B) = 공유 epilogue trampoline | [ghidra-input-dispatcher-and-objectb-instance-2026-05-18.md](ghidra-input-dispatcher-and-objectb-instance-2026-05-18.md) |
| **51** (2026-05-18, `c49b1503`) | ⭐⭐⭐ **FUN_818f0 74-entry JT 완전 디코드** → 9 distinct primary handlers (-16 init / -5 letter / -4 paired / -3 paired / -2/-1 UP-down / +55 guard / +57 guard / +others NO-OP). ⭐⭐⭐ **모든 8 유의미 handler 가 동일한 `task[+0xac78]` (R28 entity_state) secondary key 로 nested JT dispatch** → completes (event_code, entity_state) **2D matrix dispatcher** = ~30 leaf handlers. ⭐⭐⭐ **FUN_4ad10 = `*GOT[+0x444]` task_struct getter**. ⭐⭐⭐ **FUN_3d434 = global cleanup** (GOT[+0x284/288/28c] sub-system slot destruct). ⭐⭐ **FUN_92bf8 = cursor INC/DEC wraparound** (task[0xa280] cur + task[0xa281] max). ⭐⭐ **FUN_3c920 = mode 1-4 dispatch + entity context** (letter input for entity name). ⭐⭐⭐ **vtable[+0x54] = alloc(size_t)** + **GOT[+0x18] = ObjectB master ptr 정체 확정** + GOT[+0xb44/b48], task[+0xa32c] 신규. ⭐⭐ **FUN_75b98 mode==1 = timer arm** (curr+0x7d0 = 2s) at GOT[+0x9ac]. ObjectB methods 10 → **12** (+0x54 alloc, +0x70 time getter) | [ghidra-818f0-dispatch-matrix-and-allocator-2026-05-18.md](ghidra-818f0-dispatch-matrix-and-allocator-2026-05-18.md) |
| **52** (2026-05-18, `e2168141`) | ⭐⭐⭐ **30 leaf handler (event×state) 게임 액션 매핑** — arg=-16 = Confirm, arg=-5 = letter input, arg=-1/-2 (state 0/1) = UP/DOWN, arg=-3/-4 (state 3/9) = LEFT/RIGHT, arg=+55/+57 = menu hotkeys (state==2/8 guard). ⭐⭐⭐ **party member array @ `task[+0x9c70] + idx*0x3c`** 확정 (60B stride = R50 ObjectB alloc 크기와 일치) → `task[+0xac94] = "현재 선택된 party member ObjectB instance"`. ⭐⭐⭐ **task[+0xac78..+0xac9d] 38B entity_state record 9 sub-field** 매핑: +0x00 state / +0x01 backup / +0x02 flag / +0x08 ext_ptr / +0x0c sub_struct / +0x1c ObjectB inst / +0x20 cursor / +0x24 input_result / +0x25 limit. ⭐⭐⭐ **FUN_77c78 정정 = save/load record comparator** (R51 "behavior installer" 가설 폐기): vtable[+0x54] alloc 16B 2회 + FUN_99a9c query (신규) + FUN_9f624/d060/5610c (신규 3 helpers) + 16-byte name 비교. ⭐⭐⭐ **FUN_9a008 (8.8KB) = NOT battle interpreter** — 23 cmp + 28 BL = sparse state machine, 2 nested JTs (cmp #6=7entries + cmp #0xf=16entries), task[+0xb4] sub-struct base, top helper FUN_439a0 (신규, x7) → menu/status screen 후보. ⭐⭐ **신규 helper 8개** (FUN_99a9c/9f624/d060/5610c/439a0/442e4/47a14/7a49c) | [ghidra-818f0-leaves-and-record-comparator-2026-05-18.md](ghidra-818f0-leaves-and-record-comparator-2026-05-18.md) |
| **53** (2026-05-18, `1a90c54b`) | ⭐⭐⭐ **FUN_9a008 정정 = 7-mode script bytecode interpreter** (R52 "sparse state machine" 가설 폐기). JT_1 (7 entries @ 0xacbd0) + 각 leaf 의 mode-specific sub-JT (size 0xf/0x10/0x11/0xd/0x1e/0xd/0xb = **122 sub-opcode slots**). mode 4 (31 entries) dominant. 공통 setup pattern = "advance IP by 8" → bytecode VM 확정. ⭐⭐⭐⭐ **MD5 algorithm 발견 in Hero3** — FUN_5613c (Init) literal pool = `0x67452301/0xefcdab89/0x98badcfe/0x10325476` (MD5 magic constants A/B/C/D). FUN_5613c/56164/561dc/5610c = MD5 trio + wrapper `void md5(data, len, out16)`. ⭐⭐⭐ **FUN_77c78 정정 = MD5-verified save record reader** (R52 "16B name compare" → "16B MD5 digest compare"). 피처폰 save data tampering 방지. ⭐⭐⭐ **FUN_99a9c = ObjectB storage iterator** (vtable[+0x7c] size + vtable[+0x80] read) — JVM RecordStore API mapping 추정. ObjectB methods 12 → **14**. ⭐⭐ **FUN_439a0 = FUN_9a008 의 opcode dispatcher helper** (2372B, 7 calls). 신규 GOT slots +0x68, +0x70. 신규 task field +0x29e (interpreter active flag). **전투 시스템 여전히 미발견** | [ghidra-9a008-interpreter-and-md5-saveload-2026-05-18.md](ghidra-9a008-interpreter-and-md5-saveload-2026-05-18.md) |
| **54** (2026-05-18, `fb119e2f`) | ⭐⭐ **mode 4 의 31 entries → 단 8 distinct leaf** (22 epilogue) — active states 2/3/4-5/6/7/9/10/30, **state 7/10 = GOT[+0xd28], state 9 = GOT[+0xd38]** = 인접 16B 간격 paired storage system (★ player party vs enemy 버퍼 후보). ⭐⭐⭐ **task[+0x9bb4] wide-scan 71 sites / 19 readers** — top reader FUN_9ada4 가 실제로는 **FUN_9a008 mode 2 의 sub-leaf** (Ghidra 오분류). mode 2 = status condition tester 가설. ⭐⭐⭐ **task[+0x9c70] wide-scan 44 sites / 27 readers** — 신규 dispatcher 3개 발견: **FUN_3a028 (500B 16-JT party stats menu) / FUN_630e8 (3.9KB render driver) / FUN_88a30 (1.2KB task[+0xa848] sub-dispatcher)**. ⭐⭐ **전투 시스템 후보 없음** — 분석된 11개 함수 모두 menu/UI/state/save dispatcher. 새 가설: **전투가 SCN bytecode opcode 내부에 임베디드** (FUN_8e89e R35 SCN 또는 FUN_9a008 의 leaf). known funcs ~141 (1,433 entries 중 ~10%). GOT slots 25 → **27** | [ghidra-mode4-jt-and-battle-search-2026-05-18.md](ghidra-mode4-jt-and-battle-search-2026-05-18.md) |
| **55** (2026-05-18, `c0aa025d`) | ⭐⭐⭐ **GOT[+0xd28/+0xd38] 가설 완전 폐기** — R54 "player vs enemy paired buffer" 가설 잘못. 두 슬롯은 **binary 내부 asset path string table 의 인접 entries**: `/menu/chatacterheader_txt` + `/menu/chatacterbody_txt` (typo). FUN_9a008 mode 4 = **character data 화면 로딩 모드**. ⭐⭐⭐ **arith-heavy leaf grep**: 1,433 함수 entries 중 30 candidates → Top 6 후보 (FUN_95408 34 muls / 95a64 26 / 4de34 21 / 26130 14 / 4f358 10+34 asrs / 47814 13) 모두 **단일 패턴 (`r2 *= 0x3c4; r3 *= 0x3c`) = R14 NPC table grid indexing**. arith-heavy = **NPC table multi-lookup 함수**, NOT damage formula. ⭐⭐ **FUN_4f358 asrs = int16 sign-ext + cmp #0xc** → **신규 가설: enemy stats 가 NPC table row 의 int16 필드에 임베디드** (HP/atk/def/lvl). ⭐⭐⭐ **전투 시스템 미발견 — 확정된 negative result**: R47-R55 의 12개 분석 함수 모두 NPC table accessor / menu dispatcher / asset loader. **전투가 NPC table data + SCN opcode 조합으로 분산** 추정 → **R56 부터 데이터 모델링으로 전환**. 부수 산출물: asset path string table 16+ entries 발견 | [ghidra-asset-paths-and-arith-grep-2026-05-18.md](ghidra-asset-paths-and-arith-grep-2026-05-18.md) |
| **56** (2026-05-18, `30afd907`) | 🆕 **데이터 모델링 시작** — ⭐⭐⭐⭐ **work/h3/extracted/dat/ 폴더 전체 enumeration**: `enemy_dat` (5495B), `enemyh_dat` (hard mode), `enemyg_dat` (3542B graphics), `char_dat` (348B), `drop_dat` + `droph_dat` (암호화), `getitem_dat` (암호화), `i0_dat`~`i18_dat`. ⭐⭐⭐⭐ **enemy_dat 구조 완전 분석**: header(3) + name+@ (EUC-KR, name_len) + **19B stat block** + trailer(`01 1e`). 각 파일 **161 enemies** 정확히 파싱. ⭐⭐⭐⭐ **19B stat block 필드 매핑 (가설)**: lvl(byte) + pad(3) + 6×int16 BE (MP / **HP** / Gold / **ATK** / **DEF** / **EXP**) + AGI(byte). Easy vs Hard 비교 4.05x (HP), 9.75x (EXP), 13.7x (Gold), 4x (DEF) scaling. HP max=28520 (boss), EXP max=6433. ⭐⭐⭐ **R55 NPC table 가설 정정**: `task[+0x9e28]` = **runtime 에서 enemy_dat 로드한 in-memory copy**. 0x3c4 stride + 0x3c (60B) 변형 = R52 vtable[+0x54] alloc(60B) ObjectB instance 와 동일 크기 = **enemy instance object** | [ghidra-enemy-dat-and-battle-data-2026-05-18.md](ghidra-enemy-dat-and-battle-data-2026-05-18.md) |
| **57** (2026-05-18, `2d740a20`) | ⭐⭐⭐⭐ **binary 25 dat path strings**: `/dat/enemy_dat`(0xa63a4), `/boss/boss_dat`, `/dat/quest_00_dat`, `/dat/shop_dat`, `/dat/smith_dat`, `/skill/s4_dat`, `/npc/npcg_dat`, `/dat/des_dat`(0xac584), `/dat/char_dat` 등. ⭐⭐⭐⭐⭐ **DES 키 `"0EP@KO91"` 발견** @ binary 0xac594 (Hero5 와 동일 키). ⭐⭐⭐⭐⭐ **`dat/des_dat` 파일 = 표준 FIPS DES 알고리즘 테이블 824B 완전 매칭** (IP/IP⁻¹/E/P/S1-S8/PC1/PC2 정확히 일치). Hero3 DES = `[binary code + dat/des_dat tables + "0EP@KO91" key]` 완전 식별. ⭐⭐ **GOT[+0x27c] = `/dat/char_dat` path ptr** 신규. **Simple ECB 복호화 실패** — drop_dat/droph_dat/getitem_dat 가 ECB+key 그대로는 안 풀림. mode/parity/endian 변형 필요 (R58 후속). 신규 데이터 파일 발견 (binary 내 string 만): `/boss/boss_dat`, `/dat/quest_00_dat`, `/dat/shop_dat`, `/dat/smith_dat`, `/skill/s4_dat`, `/npc/npcg_dat` | [ghidra-des-system-and-dat-paths-2026-05-18.md](ghidra-des-system-and-dat-paths-2026-05-18.md) |
| **58** (2026-05-18, `a67426e5`) | ⭐⭐⭐ **R57 "JAR 재추출 필요" 가설 폐기** — 모든 파일이 이미 `work/h3/extracted/{boss,npc,skill,dat}/` 폴더에 추출되어 있음. ⭐⭐⭐⭐ **boss_dat (508B) = 15 bosses 완전 파싱**: 리츠/케이 (paired tier 1-3), 멜페토/큐 (paired tier 4), 벨루스, 시즈타이탄(×2), 아르보르, 오르도(×2), 홀리가디언 (final boss 추정 lvl 46/67). entry 구조 = header(3) + name(4, no '@') + 19B stats + **6-byte 가변 trailer**. ⭐⭐⭐⭐ **quest_*_dat 4 파일 = 44+ quests 평문 EUC-KR**: 메인퀘스트 + 사이드퀘스트 본문 (노력의 증명1, 수상한 동굴, 길잃은 소녀, 협곡의 독소, 국경 돌파, 엔자크의 영광 등). entry = (name, description, location, target, category). ⭐⭐⭐ **Hero3 게임 지역 식별**: 네메시스숲(시작), 네오솔티아, 협곡, 아스크라(적국), 엔자크사막, 토레즈(광산도시), 로우엔 평원, 리파이너의유적. ⭐⭐ **신규 DES 후보**: smith_dat / smithh_dat (896B, entropy 7.76) → 총 DES 후보 = **7 파일**. ⭐⭐⭐ **DES variant matrix 시도** (ECB / CBC+zero-IV / CBC+key-IV / parity-adjusted / bit-reversed key) = **전부 실패** → R73 에서 정정됨 (Hero5 mx_des_decrypt 변종으로 해결) | [ghidra-boss-quest-dat-and-des-variants-2026-05-18.md](ghidra-boss-quest-dat-and-des-variants-2026-05-18.md) |
| **59** (2026-05-18, `4c0dad22`) | ⭐⭐⭐⭐ **char_dat (348B) = 10 playable character classes** — **리츠와 케이가 boss 가 아닌 PLAYER 주인공** 확정. 각 5 클래스: 리츠(어쌀트워리어/디스럽터/건슬링어/나이트템플러/크레이지암즈) + 케이(버서커/데스나이트/섀도우워커/가디언나이트/소울마스터). entry = header(3) + name1(EUC-KR) + name2_len(1) + name2(class name) + 7B stat + 2B trailer. stat byte 0 = weapon type (0=검, 1=세이버, 2=도끼, 3=총, 11=특수). ⭐⭐⭐ **npcg_dat (1014B) = 78 NPC graphics info** (13B/entry 고정). 짝수 index = 변형, 홀수 = 본체 추정. ⭐⭐⭐ **s4_dat (894B) = 15 skill entries = 창수 클래스 스킬 트리** — 창술 1-7 passive + 섬광/자격/압도/유도/장벽/태산/의지/정신 8 active. ⭐⭐ **Hero3 게임 시스템 완전 매핑**: 2 주인공×5 클래스, 161×2 enemies, 15×2 bosses, 78 NPCs, ~150 skills, 44+ quests, 8 regions | [ghidra-char-npcg-skill-parsing-2026-05-18.md](ghidra-char-npcg-skill-parsing-2026-05-18.md) |
| **60** (2026-05-18, `7b855de2`) | ⭐⭐⭐ **skill_dat 7 파일 모두 평문 파싱** (s4~s10, 105 skills): 창술/검술/단도/사격/격발/영탄/광아 7 weapon classes × 15 skills (7 passive + 8 active). char_dat 의 10 playable classes 와 100% 매핑. ⭐⭐⭐ **boss_dat HP 위치 정밀 확정** — R58 의 "24-bit BE HP @ +0x08..+0x0a" 가설 폐기. 6 Ritz boss brute-force 검증으로 **+0x0a..+0x0b (BE16) = MaxHP**, +0x0c..+0x0d = CurrentHP, +0x0e..+0x0f = EXP/Gold. boss = enemy 의 단순 superset (6B 가변 trailer 만 차이). HP scaling normal→hard 2.1~2.5x. ⭐⭐⭐⭐ **menu/dat string table 6 파일 = 246 strings 평문 파싱**: chatacterhader_txt (10 클래스) / chatacterbody_txt / countryheader_txt (4 국가) / helpbody_txt (13 도움말) / **dat/InGame_txt (196 UI 텍스트)** — i18n 196개 UI 어휘의 원본 위치 확정. ⭐⭐⭐⭐ **dat/i0~i18 = 게임 전체 아이템 카탈로그 (17 파일 480+ items)**: i0=모자(33) / i1=갑옷(41) / i2=장갑(37) / i3=신발(38) / i4=창(25) / i5=대검(25) / i6=단검(25) / i7=건(25) / i8=라이플(25) / i9=다크마법(25) / i10=홀리마법(25) / i11=방패(22) / i12=반지(40) / i13=?(35) / i14=희귀(46) / i16=enchant옵션(15) / i17=퀘스트(21) / i18=소비(26 — 포션/엘릭서). ⭐⭐⭐ **i15_dat = 신규 8번째 DES 후보** (7400B, entropy 7.97, 헤더 `6a 02 87 9c 6d b8 09 76`) | [ghidra-round60-skill-item-strings-bosshp-2026-05-18.md](ghidra-round60-skill-item-strings-bosshp-2026-05-18.md) |
| **61** (2026-05-18, `b8da2617`) | ⭐⭐⭐⭐ **18 item 카테고리 body 디코드 완료** (480+ items): 장비류 (i0~i11) = 20B fixed (`price LE16 + tier byte + variant byte + req_level byte + stat_primary LE16 + stat_secondary LE16`). 무기 ATK 곡선 quantified: 창 base 43 +10/tier / 검 51 +12 / 단검 47 +11 / 건 40 +10 / 라이플 59 +13 / 다크 60 +13 / 홀리 45 +9. ⭐⭐⭐ **i12 반지 (40) bonus_type 매핑** (18B layout): type 2=HP / 3=HPRegen / 5=STR / 6=INT / 7=VIT / 10=AGI / 14=명중 / 15=회피 / 18=ATK. ⭐⭐⭐⭐ **i13_dat = 패시브 스크롤 (35)** 확정: variable body with effect_type LE16 + value LE16. ⭐⭐⭐⭐ **i14_dat = 조합/제련 재료 (46)** 확정: 붉은/푸른/투명용액 / 제련석 / 연마가루 / 정령석 / 탄성제 / 강화제. → smith_dat (DES) 가 이 재료들의 레시피 master = R60 DES 8 파일과 직접 연관. ⭐⭐⭐ **i16 enchant (15) 4B tail 분석**: 투신의/공명의/뇌제의/금강의/정령의/사신의/영제의/철벽의/속박의/결의의. ⭐⭐⭐⭐ **i18 소비 (26) effect_type 명확화**: 0x0112/13/14/15 = HP회복 4-tier (200/600/1500/3000) / 0x0416/17/18 = SP회복 3-tier (20%/50%/100%) / 0x261a = 마을귀환. ⭐⭐⭐ **skill_dat 7 파일 105 skills body 디코드**: SP cost 7-tier (100/200/.../800). weapon mastery base damage: 창 3 / 검 42 / 단검 41 / 사격 45 / 격발 45+105 / 영탄 64 / 광아 64. 특수 rank: 난무=15, 나락=5 | [ghidra-round61-item-skill-body-decode-2026-05-18.md](ghidra-round61-item-skill-body-decode-2026-05-18.md) |
| **62** (2026-05-18, `f3a75650`) | ⭐⭐⭐⭐ **R61 "trailer 4B = padding" 가정 폐기** — 346 equip items 중 **177 (51.2%)** 가 trailer 에 (bonus_type, value) × 2 쌍 보유 (반지 i12 인코딩 재사용). 신규 미분류 type code 8개 (0x01/0x03/0x04/0x08/0x09/0x0b/0x10/0x11) → R63 매핑 후보. ⭐⭐⭐⭐ **이름 prefix = rarity class** 7등급: `\|` magic 85 / `'` legendary 20 / `$` epic 40 / `{` boss_drop 29 / `@` endgame 16 / `}` quest_reward 12 / normal 144. ⭐⭐⭐ **variant byte = sprite index** 확정 — 0xff default 188회 + 슬롯별 sequential 범위. ⭐⭐⭐ **skill rank progression byte = +0x1d** (tail 마지막), 1~5 범위. ⭐⭐⭐ **cross-weapon discriminator**: 대검 `+0x0b=40` (양손 2배) / 건 `+0x0c=20` (burst) / 라이플 `+0x0f/+0x11` (range/piercing) / 홀리석 `+0x02=30` (max ATK). ⭐⭐⭐ **i17 21 quest items 중 20개 quest_*_dat 텍스트 매칭** — 시그널펜던트/협곡의성수/토레즈시민증/굴베이그의완드/일레느의노트 등. 미매칭 1개 (반토막난 지도) = 튜토리얼 시작 아이템 추정 | [ghidra-round62-item-skill-rank-quest-xref-2026-05-18.md](ghidra-round62-item-skill-rank-quest-xref-2026-05-18.md) |
| **63** (2026-05-18, `285449c8`) | ⭐⭐⭐⭐⭐ **R62 의 미식별 8 bonus_type code 전부 확정**: i16 enchant tail[0] 이 i12 ring / i13 effect_type high byte / equip trailer 와 모두 **동일한 master stat enum 사용**. 각 enchant 의 한국어 desc 가 코드 의미를 직접 노출 — `0x0c=CRI_RATE (속박의)` / `0x0d=CRI_DEF (결의의)` / `0x0e=SP_COST_REDUCE (현자의)` / `0x0f=SP_REGEN (마도의)` / `0x10=HP_DRAIN (흡혈의)` / `0x11=CD_REDUCE (폭풍의)` / `0x12=SHIELD_PIERCE (직격의)` / `0x00=ATT1_BASE (뇌제의)`. ⭐⭐⭐⭐ **R61 의 일부 라벨 정정**: 0x0c ≠ P.DEF (실제 CRI_RATE) / 0x0d ≠ M.DEF (실제 CRI_DEF). 실제 P.DEF/M.DEF/ACC/DOD/ATT1 = 0x07/0x08/0x09/0x0a/0x05 (i13 effect 확정). ⭐⭐⭐⭐ **i12 ring + equip trailer + i13 effect + i16 enchant = 네 소스가 같은 enum cross-validate**. ⭐⭐⭐ **rarity 는 stat-driven, NOT price-driven**: boss_drop 무기 0.03x = 사실상 무료 loot. quest_reward 모두 가격 0. ⭐⭐ **skill rank @ +0x1d 는 "skill power class"**: 단검 난무 r15 / 건 난사 r10 / 라이플 연쇄·다크 나락 r5 = ultimate skills | [ghidra-round63-stat-enum-final-2026-05-18.md](ghidra-round63-stat-enum-final-2026-05-18.md) |
| **64** (2026-05-19, `2a0bc9f9`) | ⭐⭐⭐⭐⭐ **`work/h3/game_balance.json` (537KB, schema v1.0) 생성** — R56-R63 의 모든 recon 산출물을 master stat enum 으로 통일된 JSON. Android 리메이크가 직접 import 할 single source of truth. 24 stat codes + 6 rarity classes + 529 items × 18 cat + 105 skills × 7 weapon + 161×2 enemies + 15×2 bosses + 4 quest files + 10 char classes + 8 DES pending 통합. ⭐⭐⭐⭐ **value scale 완전 매핑**: i12 ring / i16 enchant / equip trailer = **flat** / i13 stat buff (high 0x05-0x12) = **ratio %** (끓어오르는피 40 = +40%) / i13 HP·SP heal = **flat delta** / i13 적 대상 디버프 = **signed int16 음수** / i18 SP heal = **ratio×10** (과일쥬스 200 = 20%). ⭐⭐⭐ **effect_type low byte = target/duration enum**: 0x02 self / 0x03 target / 0x04 party / 0x12-15 HP heal t1-4 / 0x16-18 SP heal. ⭐⭐⭐ **0x14 / 0x19 미사용 확정** in i*_dat / s*_dat. ⭐⭐⭐ **Ultimate skill `+0x13 = 0x7f` sentinel byte 발견** — 4 ultimate (난무/난사/연쇄/나락). `+0x14..+0x1c` 9B = multi-hit / chain effect mask 후보 (R65) | [ghidra-round64-balance-export-value-scale-2026-05-19.md](ghidra-round64-balance-export-value-scale-2026-05-19.md) |
| **65** (2026-05-19, `c3fd598f` 흡수) | ⭐⭐⭐⭐⭐ **skill +0x10..+0x1d 14-byte effect block 완전 디코드** — primary damage scale @+0x10..+0x11 LE16 + pad @+0x12 + **1차 debuff code @+0x13 (0x7f sentinel = no debuff)** + **LE16+LE16 signed value @+0x14..+0x17** + 2차 debuff @+0x18 + value @+0x19..+0x1c + rank @+0x1d. 24 active_attack 중 9 개 debuff 보유. ⭐⭐⭐⭐ **debuff code 가 별도 enum** 확정 — 6 distinct codes: 0x03=BLEED / 0x06=ATT2_DEBUFF (망각) / 0x08=M_DEF_DEBUFF (전율) / 0x09=ACC_DEBUFF (격광/압도) / 0x0a=DOD_DEBUFF (전율 2차) / 0x15=TAUNT (유도, R63 미식별 신규) / 0x1c=STUN (참혼/저격). ⭐⭐⭐⭐ **0x14 / 0x19 = completely unused** — R64 추정도 폐기. R63 master enum 24 codes 중 실제 22 codes 만 사용. ⭐⭐⭐ **boss_dat 6B trailer 디코드**: [0] combat rating / [1] sprite_idx (8 distinct) / [2..5] 4 boss-specific skill slot IDs (0xFF = unused). 16 story boss + 14 misc boss. ⭐⭐⭐ **R64 'signed int16 통일' 가설 정정** — 실제로는 **단 2곳만 signed** (i13 effect_value 5 cases + skill +0x14..+0x17 debuff mask 9 cases). ⭐⭐⭐ **rarity × bt cross-tab**: magic 균등 / epic HP·ATT / boss_drop ATT2·ATT1 / legendary HP_DRAIN | [ghidra-round65-trailer-effect-mask-signed-2026-05-19.md](ghidra-round65-trailer-effect-mask-signed-2026-05-19.md) |
| **66** (2026-05-19, `e38c13c2`) | ⭐⭐⭐⭐⭐ **R65 의 'debuff = 별도 enum' 가설 정정 → 'stat enum 동일, value 부호로 buff/debuff'**: i13 디버프 5 cases + skill 디버프 9 cases 가 동일 enum 사용 확인. 2 컨텍스트 분리 코드 (0x0d: i13 CRI_DEF vs skill STUN_RESIST_DEBUFF / 0x1c: i13 REVIVE vs skill STUN_TRIGGER) + 1 신규 master enum (0x15 TAUNT). ⭐⭐⭐⭐⭐ **skill effect block schema v2** — R65 의 "+0x13 d1, +0x18 d2" 부분 오류. 실제는 **right-justified chain** (pos 14/19/24 = slot1/2/3, pos 29 = rank). 전율 (s9_dat) = **3-debuff skill** 신규 발견 (P_DEF + M_DEF + DOD). ⭐⭐⭐⭐ **skill debuff distinct codes 6 → 11**: 신규 5개 (0x05 ATT1 망각, 0x07 P_DEF 전율, 0x0b BLOCK 유도, 0x0d STUN_RESIST_DEBUFF 위협, 0x15 TAUNT 유도). 유도 = 자기 BLOCK +5 + TAUNT 동시 = 탱커 능력. ⭐⭐⭐⭐ **boss combat_rating 공식**: `rating = round(lvl/2 + 44)` (normal) / `round(lvl/2 + 64)` (hard). 30 boss entries 모두 검증 통과. ⭐⭐⭐ **game_balance.json v1.1 출력** (537KB → 582KB): skills.*.effect_v2 + bosses.*.trailer_decoded + combat_rating_formula + stat_enum context_split + TAUNT (0x15) + skill_debuff_codes 11 distinct | [ghidra-round66-debuff-codes-combat-rating-v1-1-2026-05-19.md](ghidra-round66-debuff-codes-combat-rating-v1-1-2026-05-19.md) |
| **67** (2026-05-19, `ed407132`) | ⭐⭐⭐⭐ **skill header (+0x00..+0x0d) 완전 디코드**: +0x00..01 LE16 SP cost / +0x04 primary damage base / +0x05 secondary (combo) / +0x07 utility marker (0x14) / +0x09..+0x0a animation timing / **+0x0b range/AoE (단검 20 / 창 30 / 검·마법 40 / 라이플 80 / utility 100)** / **+0x0c weapon flag (1=attack, 31=gun marker s7 전용 4 skills, 0=utility)** / +0x0d hit flag. ⭐⭐⭐⭐ **enemy_dat 2B trailer 의미 확정**: [0]=difficulty marker (`01`=normal / `02`=hard / `05`=cross-mode unchanged / `00`=sentinel), [1]=encounter type (`0x1e`=standard battle 126/161=78% / `0xff`=special/scripted 35). ⭐⭐⭐ **boss skill slot ID (1..20) 매핑 4 가설 분석**: H1 (글로벌 active 1-base) / H2 (weapon internal active) / H3 (weapon 15-skill 1-base) 모두 일관되지 않음. **H4 (별도 boss skill table) most likely** → R68+ binary 분석 필요. distinct IDs {1,2,3,5,7,8,9,10,13,14,19,20} | [ghidra-round67-skill-header-enemy-trailer-boss-skill-id-2026-05-19.md](ghidra-round67-skill-header-enemy-trailer-boss-skill-id-2026-05-19.md) |
| **68** (2026-05-19, `daeabed3`) | ⭐⭐⭐⭐ **gun marker 0x1f 정밀화**: s7 (건/피스톨) 만 unique flag pair (weapon_passive 0x14 + active_attack 0x1f). 라이플 (s8) 등 다른 weapon class 는 standard 0x01 사용. → 단발 권총 (s7) 은 별도 hit/ammo 시스템 보유. ⭐⭐⭐ **boss skill table 검색 결과**: R67 의 H4 가설 검증. 6 boss skill patterns 모두 binary 직접 매칭 0 hit. dat 파일 매칭은 enemyg_dat 케이 패턴 (2,2,1,1) 3 hits 만 (sprite coincidence 추정). H4 강화 but unresolved → DES 8 파일 복호화 후 재시도 필요. 신규 가설 H5/H6/H7 (tier 별 AI / stat boost / behavior weight). ⭐⭐⭐ **FUN_4f358 본문 ARM disasm 재확인**: R55 의 NPC table multi-lookup 함수. 0x3c4 row stride + 0x3c element size + task[+0x9e28] NPC table base. **boss skill mapping 과 직접 관련 없음**. **자동 분석 한계 도달** — 남은 작업 거의 DES 복호화 의존 | [ghidra-round68-boss-skill-search-gun-marker-fun4f358-2026-05-19.md](ghidra-round68-boss-skill-search-gun-marker-fun4f358-2026-05-19.md) |
| **69** (2026-05-19, `615cfe5d`) | ⭐⭐⭐⭐ **R68 's7 별도 ammo 시스템' 가설 정정**: i14 "탄성제" desc = "권총/라이플의 조합에 사용" 발견 → **권총 (s7) + 라이플 (s8) ammo 시스템 공유**. 0x1f marker (s7) vs 0x01 (s8) = **사거리/조준 모드 표시만**. 단발 권총 = 빠른 다중 타겟, 연발 라이플 = 정확 관통. "시몬의문장" = 총기 전용 강화 (s7+s8 모두). ⭐⭐⭐⭐ **i14 46 entries 7 카테고리 분류**: 공통 용액 3 / 공정 재료 6 / 원소 속성 4 / 몬스터 드롭 15 / 고대 재료 3 tier × 4 type = 12 / 클래스 강화 문장 5 (아벨=전사, 시몬=총기, 포프=마법, 부폰=방어, 하피=회피). weapon-class crafting map 완전 정리. ⭐⭐⭐ **enemy_dat 19B field scaling 정밀** (161 normal vs hard pairs): hp_max ~2.22x stable, exp_gold ~6.92x median (group별 9.7x or 1.80x), f16 ATK ~2.93x, f17 AGI ~1.21x (+2 constant). ⭐⭐⭐ **dialogue corpus 9,740/9,741 meaningful Korean**, 34,043 chars total. **LLM 번역 비용 추정 $4.09** (Claude Sonnet 4.6). **자동 분석 완전 한계 도달** — R70+ 는 모두 사용자 환경 필수 | [ghidra-round69-ammo-enemy-stat-dialogue-2026-05-19.md](ghidra-round69-ammo-enemy-stat-dialogue-2026-05-19.md) |
| **70** (2026-05-19, `940f7195`) | ⭐⭐⭐⭐⭐ **`docs/h3/MASTER_SPEC.md` 작성** — R56-R70 의 모든 발견을 단일 reference 로 통합. **15 section 4,700+ lines**. Android 리메이크 개발자 entry point. stat enum 24 codes 표 + value scale 규칙 + rarity 7 prefix + 18 item category + 105 skill + enemy/boss/quest/i14 crafting 전체. **R70 = 자동 분석 완전 종결 선언**. ⭐⭐⭐⭐ **exp_gold 4 그룹 분포 발견**: 9.7x (41 일반 전투, 가드/워리어/템플러) / 1.8x (22 정찰/고급, 로그/체이서/말벌) / **stable (16 보스/특별, `{` prefix 4개 포함 normal≈hard≈2,600)** / other (82 mixed). implicit enemy tier 시스템 — Android 리메이크 spawn 균형용. ⭐⭐⭐ **enemyg_dat 케이 패턴 sprite coincidence 확정** — R68 의 dat 파일 1 hit 정밀 검증. 22 byte sprite entry 의 byte 7-10 위치 = animation frame data. boss skill mapping 과 무관 확정. → boss skill ID 매핑은 binary/dat 어디서도 hard-coded 안 됨, H4 가설 confirm but DES 복호화 필수 | [ghidra-round70-master-spec-exp-groups-2026-05-19.md](ghidra-round70-master-spec-exp-groups-2026-05-19.md) |
| **71** (2026-05-19, `ba002199`) | ⭐⭐⭐⭐⭐ **Hero3Catalog 19 data classes + Hero3CatalogLoader** 구현 — R64-R70 의 game_balance.json (582KB v1.1) 을 typed Kotlin object 로 노출. MASTER_SPEC §13 의 "Android 리메이크 권장 구현 순서" 2번 data loader 완성. Hero4Catalog 와 동일 패턴. ⭐⭐⭐⭐ **AndroidAssetReader (Hero3 app)** — engine-core 의 platform-agnostic AssetReader interface 구현. ⭐⭐⭐⭐ **android/app/src/main/assets/game_balance.json 복사** — Hero3 app 이 직접 import 가능 (582,116 bytes). ⭐⭐⭐ **Hero3CatalogLoaderTest 12 unit tests 모두 통과** — 24 stat enum / 18 item / 7 skill set / 161 enemy / 15 boss / 6 rarity / 8 DES pending / boss combat_rating formula (30 entries 모두 match) / stat lookup. build config: testImplementation `org.json:json:20240303` 추가 (Android stub null 회피) | [ghidra-round71-catalog-loader-2026-05-19.md](ghidra-round71-catalog-loader-2026-05-19.md) |
| **72** (2026-05-19, `54e14071` 흡수) | ⭐⭐⭐⭐⭐ **MainActivity 에 Hero3Catalog lazy 로더 통합** — `MainActivity.catalog: Hero3Catalog by lazy { Hero3CatalogLoader.load(AndroidAssetReader(this)) }`. 모든 scene 에서 접근 가능, lazy 로딩 (TitleScene 진입 시 비용 0). SceneRequest.CatalogViewer 추가. ⭐⭐⭐⭐⭐ **CatalogViewerScene 신규 작성** — R71 catalog 의 raw 데이터 7-tab brower: Overview / Stat Enum (24/25 codes) / Rarity (6 prefix) / Item Categories (18) / Skill Sets (7 weapon) / Boss Roster (15 + combat_rating + skill_slots) / DES Status (8 pending). 한국어/영어 i18n. 메인 메뉴 debug 항목에 추가. ⭐⭐⭐⭐ **BestiaryScene 에 boss combat_rating 표시 추가** — 선택된 enemy 가 catalog.bossesNormal 과 매칭되면 "★ 보스 권장 lvl: 51" + "sprite #0 story/misc" 추가 표시. Context graceful degrade. ⭐⭐⭐ **build 성공 + 12 unit tests 모두 통과** | [ghidra-round72-scene-integration-2026-05-19.md](ghidra-round72-scene-integration-2026-05-19.md) |
| **73** 🏆 (2026-05-19, `55c5dc59`) | 🏆⭐⭐⭐⭐⭐ **DES 8/8 파일 복호화 성공** — Hero5 의 `mx_des_decrypt` 변종 (`tools/h5_des.py` Python 포팅) + Hero3 기존 키 `"0EP@KO91"` (R57) 조합. **R58 의 "표준 5 변종 모두 실패 → NDK runner 필수" 가설 완전 정정**. 1년 묵은 사용자 환경 의존 작업이 30분 만에 해결. 🎉⭐⭐⭐⭐⭐ **i15_dat (7,400B) = master shop catalog 확정** — 한글 "**붉은머리띠**, **레벨 10 투구; 머리띠; 보스용도 15년;**" 등 shop NPC description 텍스트 직접 발견. entry = (size, name, description body). ⭐⭐⭐⭐ **나머지 7 파일도 평문 검증 통과** — getitem_dat (7.4→3.62) / smith_dat (7.76→3.84) / smithh_dat / shop_dat / shoph_dat (3.6-4.1) / drop_dat (7.9→5.97, 18-byte stride + `11 00` separator) / droph_dat. ⭐⭐⭐⭐ **SMAF→OGG pipeline 스크립트 + 설치 가이드 작성** — 33/33 SMAF 헤더 검증 통과. graceful detection. **사용자 신뢰도 정책 ("신뢰도 높은 것만") 으로 도구 설치 보류** — R74 Phase B 에서 winget + pip + Pure Python SMAF→MIDI 자체 구현 경로로 진행 결정 대기. ⭐⭐⭐ **MASTER_SPEC §10 갱신** — DES 8 pending → **0 pending**. 핵심 lesson: **벤더 공통 cipher = cross-game 1순위 시도** (Hero4 R68 의 lesson 이 Hero3 에도 적용) | [ghidra-round73-des-success-smaf-pipeline-2026-05-19.md](ghidra-round73-des-success-smaf-pipeline-2026-05-19.md) |

### 현재 게임 시스템 모델 (Round 73 시점, 검증 vs 가설)

> ⚠ **이 섹션은 Round 18~47 의 GVM/엔진 리버싱 누적 모델** (FUN_818f0 / ObjectB / task_struct cluster 등 의 함수 단위 분석). **R55 의 "전투 시스템 없음 = 확정된 negative result"** 이후 **R56 부터는 데이터 모델링으로 전환** — game_balance.json / Hero3Catalog / DES 8 파일 평문 등의 데이터 도메인 모델은 ★★★★★ [`MASTER_SPEC.md`](MASTER_SPEC.md) §1~§15 (15 sections / 4,700+ lines) 가 single reference. Android 리메이크 개발자는 MASTER_SPEC 부터 보면 된다. 아래 모델은 **엔진/플랫폼 레이어 (GVM bytecode + task_struct + 4 known objects + 7 indirect entries)** 의 reference 로 유지.
>
> R51-R73 누적 갱신:
> - **R51-R52** — FUN_818f0 = (event_code, entity_state) 2D matrix dispatcher (74-JT × secondary state) → 30 leaf handler. ObjectB methods 10 → **14** (+0x54 alloc / +0x70 time getter / +0x7c size / +0x80 read).
> - **R53** — FUN_9a008 = **7-mode script bytecode interpreter (122 sub-opcodes)** (NOT battle). MD5 algorithm 발견 (FUN_5613c~561dc, save data tampering 방지). FUN_77c78 = MD5-verified save record reader.
> - **R54-R55** — FUN_9a008 mode 4 31 entries → 8 distinct leaf. GOT[+0xd28/+0xd38] paired storage 가설 폐기 (사실은 asset path string table). arith-heavy leaf grep → 모두 NPC table multi-lookup (R14 0x3c4×0x3c grid), **전투 시스템 없음 = 확정 negative result**.
> - **R56+ 데이터 모델링** — enemy_dat (161×2 19B) + boss_dat (15 + 6B trailer) + char_dat (10 playable: 리츠/케이 각 5 class) + skill_dat (7 파일 105 skills) + item dat (18 카테고리 480+ items) + i14 crafting (46 entries 7 카테고리) + DES (R57 키 `"0EP@KO91"` 식별 → R73 8/8 복호화 성공: Hero5 mx_des_decrypt 변종). 정밀 정보는 MASTER_SPEC 참조.

**✅ 검증된 사실 (R18~R47 GVM/엔진 모델 누적)** (실측 disassembly + reader 통계):
...
**🔬 가설 단계 (Round 47 → Round 73 갱신)**:

✅ **검증 완료** (R47 → R73 까지 정정/확정된 가설):
- ~~task_struct[0x9bb4] = state machine state~~ → Round 25: bit flag field
- ~~0x9c70 = byte array base~~ → Round 25: 단순 byte field, **Round 52 정정: party member array @ `task[+0x9c70] + idx*0x3c`** (60B stride, ObjectB instance)
...
- ~~task[0xa848]+0x08 의미~~ → Round 50: **동적 할당된 ObjectB 인스턴스 포인터** (60B, vtable[+0x54] alloc / +0x58 destruct)
- ~~FUN_818f0 = entity iteration loop~~ → Round 50: **74-entry JT 입력 디스패처** (`(arg+0x10) ≤ 0x49 unsigned` = signed [-0x10..0x39]), Round 51: **(event_code, entity_state) 2D matrix dispatcher**
- ~~FUN_9a008 = battle interpreter~~ → Round 53: **7-mode script bytecode interpreter** (122 sub-opcodes, NOT battle)
- ~~FUN_77c78 = behavior installer / 16B name compare~~ → Round 53: **MD5-verified save record reader** (16B = MD5 digest)
- ~~task[0x9c71..0x9c76] 6-byte cluster = "party member"~~ → Round 52 정정: **6 byte sub-flags of entity_state record**, party array 실제로는 task[+0x9c70] + 0x3c stride
- ~~GOT[+0xd28/+0xd38] = player vs enemy paired buffer~~ → Round 55 폐기: **asset path string table 인접 entries** (`/menu/chatacterheader_txt` + `/menu/chatacterbody_txt`)
- ~~전투 시스템 존재~~ → **Round 55 확정 negative result: 전투 시스템 없음** — NPC table data + SCN opcode 안에 분산. R56 부터 데이터 모델링으로 전환
- ~~DES "표준 5 변종 모두 실패 → NDK runner 필수" (R58)~~ → **Round 73: Hero5 mx_des_decrypt 변종 + key `"0EP@KO91"` 로 8/8 복호화 성공** (1년 묵은 가설이 30분에 해결)
...
- **boss skill ID H4 가설** — R67 H4 (별도 boss skill table) 가장 가능성 높음. R74 Phase A 의 drop_dat / smith_dat 평문 분석에서 distinct IDs {1,2,3,5,7,8,9,10,13,14,19,20} 매핑 후 검증 필요
- **i14 ammo 시스템 정확한 작동** — R69 의 권총/라이플 공유 ammo 가설 (탄성제 desc). 실 게임 효과 미확인
- **callback queue records 의 destination 함수들** — Stage 1 record[+0x154] / Stage 2 record[+0x18] 실제 callback (런타임 결정)
- **sound dispatcher 22 arms → 33 sound files 매핑** — internal_id (sound_id - 4) → 정확한 BGM/SFX 파일 매핑 (자동 분석 외 검증 필요)
...
### Round 59 uncommitted 산출물 (인덱스)

> 마지막 commit (`a67426e5`, Round 58) 이후 Round 59 산출물이 uncommitted.

**Round 59 (2026-05-18)** — char/npcg/s4 dat 평문 파싱 + 리츠/케이 PLAYER 캐릭터 확정:
- 신규 doc: [`ghidra-char-npcg-skill-parsing-2026-05-18.md`](ghidra-char-npcg-skill-parsing-2026-05-18.md)
- 신규 scripts (1): `parse_char_npcg_s4_dat.py`
- 기타: `work/h3/round59_misc_dat_v2.txt`

**추천 commit 메시지**: `feat:영웅서기3 Round 59 — char/npcg/s4 dat 평문 파싱 (10 classes + 78 NPCs + 15 skills) + 리츠/케이 PLAYER 캐릭터 확정`

### Round 58 산출물 (이미 commit `a67426e5`)
...
> ⚠ **이 섹션은 Round 47 (2026-05-17) 당시의 R48 작업 후보 표**. 현재 R73 시점 (2026-05-19) 에서는 **이미 R48~R73 모두 완료**. 따라서 이 표는 **historical 참고용**이고, 실제 "이어서 진행" 지시 시 봐야 할 것은 ★★★★★ [`SESSION_HANDOFF.md`](SESSION_HANDOFF.md) (R74 Phase A/B/C 가이드).
>
> R47→R73 의 사건 흐름 요약:
> - R47~R50: task[0xa848] sub-struct + FUN_3a86c JT + ObjectB instance lifecycle (input dispatcher cluster)
> - R51~R55: FUN_818f0 2D matrix + FUN_9a008 7-mode interpreter + **전투 시스템 없음 negative result** 확정
> - R56~R73: **데이터 모델링 전환** — enemy/boss/char/skill/item/quest 평문 파싱 → game_balance.json + Hero3Catalog + Android scene 통합 + 🏆 R73 **DES 8/8 복호화 성공**
>
> 아래 R48 작업 표는 모두 완료되었으므로 ⚠ **참고 보존만** 함.

다음 세션에서 사용자가 "영웅서기3 이어서 진행" 같은 짧은 지시만 줬을 때, **R74 부터는 이 표가 아니라 [`SESSION_HANDOFF.md`](SESSION_HANDOFF.md) 을 본다** (Phase A = 즉시 가능 / Phase B = SMAF 도구 정책 / Phase C = LLM 번역).

**(아래는 R47 당시 작성된 R48 작업 후보 표 — 보존용)**:
